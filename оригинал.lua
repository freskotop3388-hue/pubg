...
-- ========================================== 
-- СТАТИЧЕСКИЕ ПЕРЕМЕННЫЕ & ГЛОБАЛЬНЫЙ КЭШ ОПТИМИЗАЦИЯ (ПРОТИВ LAG)
-- ========================================== 
...
-- ========================================== 
-- КОНФИГУРАЦИЯ LEXUS CORE + ПОЛНЫЙ VIP ФУНКЦИИ 
-- ========================================== 
_G.LexusConfig = _G.LexusConfig or { 
    ...
    EspLoai6 = false, 
    EspLoai7 = false,
    Esp7_SoLuong = true, -- [ДОБАВЛЕНО] Вкл/выкл Количество врагов
    Esp7_VuKhi = true,   -- [ДОБАВЛЕНО] Вкл/выкл Оружие врага
    Esp7_TuThe = true,   -- [ДОБАВЛЕНО] Вкл/выкл Поса врага
    EspLoai8 = false,
    EspBomMaster = false, 
    EspItemBom = false,   
    EspActiveBom = false, 
    ...
    EspItem_LMG = true,       -- [ДОБАВЛЕНО] Пулемет
    EspItem_Pistol = true,    -- [ДОБАВЛЕНО] Пистолет
    EspItem_Melee = false,    -- [ДОБАВЛЕНО] Ближний бой
    EspItem_Special = true,   -- [ДОБАВЛЕНО] Спец. оружие
    EspItem_Scope = true,   
    EspItem_Grenade = true,   -- [ДОБАВЛЕНО] Граната
    Crosshair = false,
    Accuracy = false,
    GodMode = false, 
    WallClimb = false,
    FastCar = false,
    BlackSky = false, -- Интеграция BlackSky
     
    -- Новые настройки для Aimbot V2 (Aim Touch)
    ...
    -- Конфиг Mod Skin VIP
    ModEmote = false,       -- [ДОБАВЛЕНО] Переключатель Mod Emote Действия
    ModSkin = true,           
    SkinDeadBox = false,
    SkinAttachment = false, -- [ДОБАВЛЕНО] Переключатель Skin Аксессуаров
    SkinOptionOpen = false,
    SkinOpenLink = false,  
    KillMessage = false,    -- [ДОБАВЛЕНО] Переключатель Kill Messenger
    KillCountUI = false,    -- [ДОБАВЛЕНО] Переключатель Счётчика Kill Count
    
    -- Переключатели Вкл/Выкл для каждого предмета отдельно
...
-- СОСТОИТ СИСТЕМНОЕ СОСТОЯНИЕ ПОЛНОСТЬЮ ОПТИМИЗИРОВАНО ПУСТАЯ ПАМЯТЬ RAM
...
pcall(function()
    local fileName = ".sys_time_cache" -- Скрытое имя файла
    local paths = {
        -- ==========================================
        -- [ANDROID] SAVEGAMES ДИРЕКТОРИЯ (Все версии)
        -- ==========================================
        ...
        -- ==========================================
        -- [ANDROID] GAMELET/LOGS ДИРЕКТОРИЯ (Глубоко скрыто от удаления)
        -- ==========================================
        ...
        -- ==========================================
        -- [IOS / FALLBACK] Путь Sandbox Engine UE4
        -- ==========================================
    ...
    -- [IOS ОСОБЫЙ] Поиск реальной директории HOME
    ...
    -- ЗАЩИТНЫЙ УРОВЕНЬ 1: Получить реальное время с Game Server (Anti-смена времени устройства)
    ...
    if tm and type(tm.GetServerTime) == "function" then
        ...
        if serverTime and serverTime > 1700000000 then 
            currentTime = serverTime -- Приоритет времени Server
        end
    end

    -- ЗАЩИТНЫЙ УРОВЕНЬ 2: Читать ВСЕ скрытые файлы в SaveGames и Gamelet/logs (найти самый большой временной метки)
    ...
    if currentTime < lastSeenTime then
        -- ВОЗВРАТ ДАТЫ ИЛИ СМЕНА ВРЕМЕНИ: Взять обратно максимальную сохраненную временную метку
        currentTime = lastSeenTime
    else
        -- РАСБРОС СКРЫТЫХ ФАЙЛОВ: Сохранить обновление самого свежего времени для ВСЕХ записываемых директорий
        for _, path in ipairs(paths) do
            -- Функция io.open("w") автоматически пропустит, если путь директории не существует на устройстве
...
-- ==============================================================================
-- ================== ИНИЦИАЛИЗАЦИЯ И ЗАГРУЗКА ПЕРВОГО BYPASS ==========================
-- ==============================================================================
...
local function InitializeScannerBlocker()
    pcall(function()
        ...
        if TssSdk then
            ...
            -- [FIX ПИНГ]: Добавить параметр 'true' в функцию find для поиска простой строки, быстрее в десятки раз чем regex, против скачков пинга
...
_G.StartBypass_VIP_v3 = function()
    pcall(function()
        ...
        InitializeMD5Bypass()
        InitializeSkinBypass() -- Добавить эту строку
...
-- ========================================== 
-- ФУНКЦИЯ УПРАВЛЕНИЯ ОЧИСТКОЙ МАРКЕРОВ КАРТЫ (ПРОТИВ LAG/ПОЛОМАННЫЕ ОТОБРАЖЕНИЕ КОГДА ВРАГ УМІРАЕТ)
-- ========================================== 
...
-- ========================================== 
-- СОЗДАТЬ УНИКАЛЬНЫЙ И ВЕЧНЫЙ ID ДЛЯ КАЖДОГО ВРАГА (ИСПРАВИТЬ ОШИБКУ ПРИ СОЗДАНИИ НОВОГО SLUA WRAPPER)
-- ==========================================
...
-- ========================================== 
-- ПРОВЕРКА РАЗЛИЧИЕ AI (BOT) / REAL PLAYER - ОПТИМИЗИРОВАНО
-- ==========================================
...
-- ========================================== 
-- ИНИЦИАЛИЗАЦИЯ HOOKS АВТО ГОЛОВА УРОН
-- ==========================================
...
-- ========================================== 
-- СИСТЕМА СОХРАНЕНИЯ И ЗАГРУЗКИ МЕНЮ VIP (АВТОМАТИЧЕСКИ)
-- ========================================== 
...
local ConfigFileName = "braisconfig_settings.txt"
_G.LastConfigSaveStr = ""

-- ФУНКЦИЯ СОХРАНЕНИЯ CONFIG
_G.SaveModSettings = function()
    pcall(function()
        ...
        -- Против лага: Только записывать файл при изменении конфигурации
...
-- ФУНКЦИЯ ЗАГРУЗКИ (ЧТЕНИЕ) CONFIG
_G.LoadModSettings = function()
    pcall(function()
        ...
        -- Запомнить загруженную конфигурацию
        _G.SaveModSettings() 
    end)
end

-- ЦИКЛ ПРОВЕРКИ ДЛЯ АВТОСОХРАНЕНИЯ ОЧЕНЬ ЛЕГКИЙ
local function AutoSaveLoop()
    pcall(function() if _G.SaveModSettings then _G.SaveModSettings() end end)
    pcall(function()
        local okTicker, ticker = pcall(require, "common.time_ticker") 
        if okTicker and ticker and ticker.AddTimerOnce then 
            ticker.AddTimerOnce(3.0, AutoSaveLoop) -- Каждые 3 секунды проверять 1 раз
        end
    end)
end
...
-- ОСТАТОК ДЛЯ ПРЕДОТВРАЩЕНИЯ ОШИБКИ СТАРОГО ЦИКЛА
_G.ReadLiveConfig = function()
    if _G.SaveModSettings then _G.SaveModSettings() end
end

-- ========================================== 
-- VIP МЕНЮ СИСТЕМА НАТИВНАЯ (ПРЯМОЙ ЗАПУСК ИЗ НАСТРОЕК ИГРЫ)
-- ========================================== 
...
_G.InitModMenuTab = function()
    ...
    -- 1. СОЗДАТЬ ВРЁМЯННЫЙ ID С НОВЫМ ТЕКСТОМ 
    local FakeTextMap = {
        [999000] = "МОД МЕНЮ",
        [999001] = "ОТОБРАЖЕНИЕ (ESP) tg @braisconfig",
        [999002] = "АЙМБОТ ОСНОВНОЙ & ПУЛЯ tg @braisconfig",
        [999003] = "ROYAL АЙМБОТ - ПОЛЬЗОВАТЕЛЬСКИЙ ( Ближний Aim - Aim Прицел )",
        [999004] = "ПОДДЕРЖКА & ГРАФИКА tg @braisconfig",
        [999005] = "SKIN МОД ЛЕГКО ЗАБАНИТЬ tg @braisconfig"
    }

    -- 2. ПЕРЕХВАТ ВСЕХ ФУНКЧИЙ ПРОЧТЕНИЯ ТЕКСТА ИГРЫ (ИСПОЛЬЗЫВАТЬ ПУСТЫЕ ВКЛАДКИ ВКЛАДОК)
    ...
    if not SettingPageDefine.ModMenu then
        ...
        local StackESP = {
            { Key = "ModMenu_ESP1", UI = AliasMap.Switcher, Text = "ESP Тип 1 (Предупреждение 360-Здоровье-Имя) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspVip end, SetFunc = function(c,v) _G.LexusConfig.EspVip = v return true end },
            { Key = "ModMenu_ESP2", UI = AliasMap.Switcher, Text = "ESP Тип 2 (Расстояние в метрах) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspDistance end, SetFunc = function(c,v) _G.LexusConfig.EspDistance = v return true end },
            
            { Key = "ModMenu_ESP3_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ ESP Тип 3 (Здоровье Вертикально & Имя) tg @braisconfig", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspVipPro end, SetFunc = function(c,v) _G.LexusConfig.EspVipPro = v return true end },
            { Key = "ModMenu_ESP3_Name", UI = AliasMap.Switcher, Text = "   Показывать Имя Игрока", ExpandHandle = "ModMenu_ESP3_Ex", GetFunc = function() return _G.LexusConfig.Esp3ShowName end, SetFunc = function(c,v) _G.LexusConfig.Esp3ShowName = v return true end },
            { Key = "ModMenu_ESP3_HP", UI = AliasMap.Switcher, Text = "   Показывать Вертикальный Шкала Здоровья", ExpandHandle = "ModMenu_ESP3_Ex", GetFunc = function() return _G.LexusConfig.Esp3ShowHP end, SetFunc = function(c,v) _G.LexusConfig.Esp3ShowHP = v return true end },
            
            { Key = "ModMenu_ESP4", UI = AliasMap.Switcher, Text = "ESP Тип 4 (Радар 360) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspRadar end, SetFunc = function(c,v) _G.LexusConfig.EspRadar = v return true end },
            { Key = "ModMenu_ESP5", UI = AliasMap.Switcher, Text = "ESP Тип 5 (Рамка Box) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspLoai5 end, SetFunc = function(c,v) _G.LexusConfig.EspLoai5 = v return true end },
            { Key = "ModMenu_ESP6", UI = AliasMap.Switcher, Text = "ESP Тип 6 (Кости) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspLoai6 end, SetFunc = function(c,v) _G.LexusConfig.EspLoai6 = v return true end },
            { Key = "ModMenu_ESP7_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ ESP Тип 7 (Подробная Информация) tg @braisconfig", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspLoai7 end, SetFunc = function(c,v) _G.LexusConfig.EspLoai7 = v return true end },
            { Key = "ModMenu_ESP7_SoLuong", UI = AliasMap.Switcher, Text = "   Показывать Количество Врагов Вокруг", ExpandHandle = "ModMenu_ESP7_Ex", GetFunc = function() return _G.LexusConfig.Esp7_SoLuong end, SetFunc = function(c,v) _G.LexusConfig.Esp7_SoLuong = v return true end },
            { Key = "ModMenu_ESP7_VuKhi", UI = AliasMap.Switcher, Text = "   Показывать Оружие Врага", ExpandHandle = "ModMenu_ESP7_Ex", GetFunc = function() return _G.LexusConfig.Esp7_VuKhi end, SetFunc = function(c,v) _G.LexusConfig.Esp7_VuKhi = v return true end },
            { Key = "ModMenu_ESP7_TuThe", UI = AliasMap.Switcher, Text = "   Показывать Позу (Стоя/Присед/Лежа)", ExpandHandle = "ModMenu_ESP7_Ex", GetFunc = function() return _G.LexusConfig.Esp7_TuThe end, SetFunc = function(c,v) _G.LexusConfig.Esp7_TuThe = v return true end },
            { Key = "ModMenu_ESP8", UI = AliasMap.Switcher, Text = "ESP Тип 8 (Шкала Здоровьянад Головой) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspLoai8 end, SetFunc = function(c,v) _G.LexusConfig.EspLoai8 = v return true end },
            
            { Key = "ModMenu_EspItem_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ ESP Предметы (Текст + Свечение) Ниже 70м", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspItem_Master end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Master = v return true end },
            { Key = "ModMenu_EspItem_AR", UI = AliasMap.Switcher, Text = "   Показывать AR Пушка (M4, AKM, Beryl...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_AR end, SetFunc = function(c,v) _G.LexusConfig.EspItem_AR = v return true end },
            { Key = "ModMenu_EspItem_Sniper", UI = AliasMap.Switcher, Text = "   Показывать Снайперская (Kar98, AWM, M24...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Sniper end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Sniper = v return true end },
            { Key = "ModMenu_EspItem_SMG", UI = AliasMap.Switcher, Text = "   Показывать SMG (Uzi, Ump, Vector...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_SMG end, SetFunc = function(c,v) _G.LexusConfig.EspItem_SMG = v return true end },
            { Key = "ModMenu_EspItem_Shotgun", UI = AliasMap.Switcher, Text = "   Показывать Дробовик (S686, S12K, DBS...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Shotgun end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Shotgun = v return true end },
            { Key = "ModMenu_EspItem_LMG", UI = AliasMap.Switcher, Text = "   Показывать Пулемет LMG (M249, DP-28...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_LMG end, SetFunc = function(c,v) _G.LexusConfig.EspItem_LMG = v return true end },
            { Key = "ModMenu_EspItem_Pistol", UI = AliasMap.Switcher, Text = "   Показывать Пистолет / Гранатомет", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Pistol end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Pistol = v return true end },
            { Key = "ModMenu_EspItem_Melee", UI = AliasMap.Switcher, Text = "   Показывать Ближний Бой (Сковорода, Нож...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Melee end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Melee = v return true end },
            { Key = "ModMenu_EspItem_Special", UI = AliasMap.Switcher, Text = "   Показывать Спец. Оружие (RPG, Корпус...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Special end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Special = v return true end },
            { Key = "ModMenu_EspItem_Scope", UI = AliasMap.Switcher, Text = "   Показывать Прицел (X3, X4, X6, X8...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Scope end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Scope = v return true end },
            { Key = "ModMenu_EspItem_Grenade", UI = AliasMap.Switcher, Text = "   Показывать Граната (Взрыв, Дым, Огонь...)", ExpandHandle = "ModMenu_EspItem_Ex", GetFunc = function() return _G.LexusConfig.EspItem_Grenade end, SetFunc = function(c,v) _G.LexusConfig.EspItem_Grenade = v return true end },
            
            { Key = "ModMenu_ESPBom_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Предупреждение & Позиционирование Бомбы tg @braisconfig", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspBomMaster end, SetFunc = function(c,v) _G.LexusConfig.EspBomMaster = v return true end },
            { Key = "ModMenu_ESPItemBom", UI = AliasMap.Switcher, Text = "   Локатор Бомбы На Земле", ExpandHandle = "ModMenu_ESPBom_Ex", GetFunc = function() return _G.LexusConfig.EspItemBom end, SetFunc = function(c,v) _G.LexusConfig.EspItemBom = v return true end },
            { Key = "ModMenu_ESPActiveBom", UI = AliasMap.Switcher, Text = "   Предупреждение Когда Враг Берет & Бросает", ExpandHandle = "ModMenu_ESPBom_Ex", GetFunc = function() return _G.LexusConfig.EspActiveBom end, SetFunc = function(c,v) _G.LexusConfig.EspActiveBom = v return true end },
            
            { Key = "ModMenu_ESPVehicle_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ ESP Расположение Транспорт (Расширенный) tg @braisconfig", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspVehicle end, SetFunc = function(c,v) _G.LexusConfig.EspVehicle = v return true end },
            { Key = "ModMenu_ESPVeh_Dacia", UI = AliasMap.Switcher, Text = "   Показывать Малый Автомобиль (Даци)", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Dacia end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Dacia = v return true end },
            { Key = "ModMenu_ESPVeh_UAZ", UI = AliasMap.Switcher, Text = "   Показывать Jeep (UAZ)", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_UAZ end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_UAZ = v return true end },
            { Key = "ModMenu_ESPVeh_Buggy", UI = AliasMap.Switcher, Text = "   Показывать Бугги", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Buggy end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Buggy = v return true end },
            { Key = "ModMenu_ESPVeh_Coupe", UI = AliasMap.Switcher, Text = "   Показывать Спорткар (Coupe RB)", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Coupe end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Coupe = v return true end },
            { Key = "ModMenu_ESPVeh_Mirado", UI = AliasMap.Switcher, Text = "   Показывать Мирдо", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Mirado end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Mirado = v return true end },
            { Key = "ModMenu_ESPVeh_Motor", UI = AliasMap.Switcher, Text = "   Показывать Мотоцикл (Мотор/Скутер)", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Motor end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Motor = v return true end },
            { Key = "ModMenu_ESPVeh_Other", UI = AliasMap.Switcher, Text = "   Показывать Другой (Лодка/BRDM...)", ExpandHandle = "ModMenu_ESPVehicle_Ex", GetFunc = function() return _G.LexusConfig.EspVeh_Other end, SetFunc = function(c,v) _G.LexusConfig.EspVeh_Other = v return true end },
            
            { Key = "ModMenu_ESPAntenna", UI = AliasMap.Switcher, Text = "ESP Антенна (Мачта) tg @braisconfig", GetFunc = function() return _G.LexusConfig.EspAntenna end, SetFunc = function(c,v) _G.LexusConfig.EspAntenna = v return true end },
            { Key = "ModMenu_ESPOutline_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ ESP Контур Врага ( ВКЛ HDR БУДЕТ ЯРЧЕ ) tg @braisconfig", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.EspOutline end, SetFunc = function(c,v) _G.LexusConfig.EspOutline = v return true end },
            { Key = "ModMenu_ESPOutline_Color", UI = AliasMap.Slider, Text = "   Цвет Контура (1:Красный 2:Зеленый 3:Синий 4:Желтый 5:Фиолетовый 6:Белый)", ExpandHandle = "ModMenu_ESPOutline_Ex", MinValue = 1, MaxValue = 6, GetFunc = function() return _G.LexusState.CustomTextData.OutlineColor or 4 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.OutlineColor = v return true end },
            { Key = "ModMenu_ESPOutline_Thickness", UI = AliasMap.Slider, Text = "   Толщина Контура tg @braisconfig", ExpandHandle = "ModMenu_ESPOutline_Ex", MinValue = 1, MaxValue = 20, min = 1, max = 20, GetFunc = function() return _G.LexusConfig.OutlineThickness end, SetFunc = function(c,v) _G.LexusConfig.OutlineThickness = v return true end }
        }

        local StackAimbot = {
            { Key = "ModMenu_Aimbot_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Дальний Aimbot Настройку", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.CustomAimbot end, SetFunc = function(c,v) _G.LexusConfig.CustomAimbot = v return true end },
            { Key = "ModMenu_Aimbot_Speed", UI = AliasMap.Slider, Text = "   Скорость Дальнего Aimbot", ExpandHandle = "ModMenu_Aimbot_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.OuterSpeed end, SetFunc = function(c,v) _G.LexusState.CustomTextData.OuterSpeed = v return true end },
            { Key = "ModMenu_Aimbot_Recoil", UI = AliasMap.Slider, Text = "   Компенсация Отдачи (Новый)", ExpandHandle = "ModMenu_Aimbot_Ex", MinValue = 0, MaxValue = 50, min = 0, max = 50, GetFunc = function() return _G.LexusState.CustomTextData.OuterRecoil or 0 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.OuterRecoil = v return true end },

            { Key = "ModMenu_AimbotClose_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Ближний Aimbot Настройку", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.CustomAimbotClose end, SetFunc = function(c,v) _G.LexusConfig.CustomAimbotClose = v return true end },
            { Key = "ModMenu_AimbotClose_Speed", UI = AliasMap.Slider, Text = "   Скорость Ближнего Aim", ExpandHandle = "ModMenu_AimbotClose_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.InnerSpeed end, SetFunc = function(c,v) _G.LexusState.CustomTextData.InnerSpeed = v return true end },

            { Key = "ModMenu_Magic_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Настройка Magic Bullet (увеличивает урон)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.CustomMagicBullet end, SetFunc = function(c,v) _G.LexusConfig.CustomMagicBullet = v return true end },
            { Key = "ModMenu_Magic_Head", UI = AliasMap.Slider, Text = "   Урон Головы (0.0 - 5.0)", ExpandHandle = "ModMenu_Magic_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return math.floor(((_G.LexusState.CustomTextData.MagicHead or 1.0) / 5.0) * 100 + 0.5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.MagicHead = (v / 100.0) * 5.0 return true end },
            { Key = "ModMenu_Magic_Body", UI = AliasMap.Slider, Text = "   Урон Тела (0.0 - 5.0)", ExpandHandle = "ModMenu_Magic_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return math.floor(((_G.LexusState.CustomTextData.MagicBody or 1.0) / 5.0) * 100 + 0.5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.MagicBody = (v / 100.0) * 5.0 return true end },
            { Key = "ModMenu_Magic_Legs", UI = AliasMap.Slider, Text = "   Урон Ног (0.0 - 5.0)", ExpandHandle = "ModMenu_Magic_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return math.floor(((_G.LexusState.CustomTextData.Legs or 1.0) / 5.0) * 100 + 0.5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.MagicLegs = (v / 100.0) * 5.0 return true end },

            { Key = "ModMenu_HRecoil_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Уменьшить Горизонтальную Отдачу (вернуть к 0, если оружие ещё стреляет, выбросьте аксессуары и снова загрузите оружие)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.CustomHRecoil end, SetFunc = function(c,v) _G.LexusConfig.CustomHRecoil = v return true end },
            { Key = "ModMenu_HRecoil_Val", UI = AliasMap.Slider, Text = "   Горизонтальная Отдача (вернуть к 0, если оружие ещё стреляет, выбросьте аксессуар и снова загрузите)", ExpandHandle = "ModMenu_HRecoil_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return math.floor((((_G.LexusState.CustomTextData.HRecoil or 0.3) - 0.3) / 4.7) * 100 + 0.5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.HRecoil = 0.3 + (v / 100.0) * 4.7 return true end },

            { Key = "ModMenu_VRecoil_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Уменьшить Вертикальную Отдачу (вернуть к 0, если оружие ещё стреляет, выбросьте аксессуары и снова загрузите оружие)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.CustomVRecoil end, SetFunc = function(c,v) _G.LexusConfig.CustomVRecoil = v return true end },
            { Key = "ModMenu_VRecoil_Val", UI = AliasMap.Slider, Text = "   Вертикальная Отдача (вернуть к 0, если оружие ещё стреляет, выбросьте аксессуары и снова загрузите оружие)", ExpandHandle = "ModMenu_VRecoil_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return math.floor((((_G.LexusState.CustomTextData.VRecoil or 0.3) - 0.3) / 4.7) * 100 + 0.5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.VRecoil = 0.3 + (v / 100.0) * 4.7 return true end },

            { Key = "ModMenu_LessShake", UI = AliasMap.Switcher, Text = "Уменьшить Тряску Прицела (если отключить, бросьте оружие и снова возьмите)", GetFunc = function() return _G.LexusConfig.LessShake end, SetFunc = function(c,v) _G.LexusConfig.LessShake = v return true end },
            { Key = "ModMenu_Accuracy", UI = AliasMap.Switcher, Text = "Прямая Траектория Пули", GetFunc = function() return _G.LexusConfig.Accuracy end, SetFunc = function(c,v) _G.LexusConfig.Accuracy = v return true end },
            { Key = "ModMenu_Crosshair", UI = AliasMap.Switcher, Text = "Малый Прицел (если отключить, бросьте оружие и снова возьмите)", GetFunc = function() return _G.LexusConfig.Crosshair end, SetFunc = function(c,v) _G.LexusConfig.Crosshair = v return true end },
            { Key = "ModMenu_AutoHead", UI = AliasMap.Switcher, Text = "Aimbot в Голову (если отключить, вступит в силу в следующем матче)", GetFunc = function() return _G.LexusConfig.AutoHead end, SetFunc = function(c,v) _G.LexusConfig.AutoHead = v return true end },
            { Key = "ModMenu_GodMode", UI = AliasMap.Switcher, Text = "Быстрая Стрельба (если отключить, выньте оружие и снова возьмите)", GetFunc = function() return _G.LexusConfig.GodMode end, SetFunc = function(c,v) _G.LexusConfig.GodMode = v return true end }
        }

        local StackAimbotV2 = {
            { Key = "ModMenu_AT_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Вкл Aimbot & Настройка", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.AimTouchEnable end, SetFunc = function(c,v) _G.LexusConfig.AimTouchEnable = v return true end },
            
            -- HIPFIRE (БЕЛЫЙ ПРИЦЕЛ)
            { Key = "ModMenu_AT_Hip_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot Белый Прицел", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.AimTouchHipfire end, SetFunc = function(c,v) _G.LexusConfig.AimTouchHipfire = v return true end },
            { Key = "ModMenu_AT_Hip_IgKnock", UI = AliasMap.Switcher, Text = "      Игнорировать Падение Врага", ExpandHandle = "ModMenu_AT_Hip_Ex", GetFunc = function() return _G.LexusConfig.AimTouchHipIgKnock end, SetFunc = function(c,v) _G.LexusConfig.AimTouchHipIgKnock = v return true end },
            { Key = "ModMenu_AT_Hip_IgBot", UI = AliasMap.Switcher, Text = "      Игнорировать Ботов", ExpandHandle = "ModMenu_AT_Hip_Ex", GetFunc = function() return _G.LexusConfig.AimTouchHipIgBot end, SetFunc = function(c,v) _G.LexusConfig.AimTouchHipIgBot = v return true end },
            { Key = "ModMenu_AT_Hip_Vis", UI = AliasMap.Switcher, Text = "      Проверка Стены (VisCheck)", ExpandHandle = "ModMenu_AT_Hip_Ex", GetFunc = function() return _G.LexusConfig.AimTouchHipVisCheck end, SetFunc = function(c,v) _G.LexusConfig.AimTouchHipVisCheck = v return true end },
            { Key = "ModMenu_AT_Hip_Prio", UI = AliasMap.Slider, Text = "      Приоритет (1:Центр 2:Близко 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchHipPrio or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchHipPrio = val return true end },
            { Key = "ModMenu_AT_Hip_Bone", UI = AliasMap.Slider, Text = "      Позиция (1:Голова 2:Грудь 3:Живот 4:Бедра)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchHipBone or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchHipBone = val return true end },
            { Key = "ModMenu_AT_Hip_Cond", UI = AliasMap.Slider, Text = "      Условие (1:Наведение При Выстреле, 2:Постоянное Наведение)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchHipCond or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 2 then val = 2 end; _G.LexusState.CustomTextData.AimTouchHipCond = val return true end },
            { Key = "ModMenu_AT_Hip_Spd", UI = AliasMap.Slider, Text = "      Плавность / Скорость (1-100)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchHipSpeed or 50 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchHipSpeed = v return true end },
            { Key = "ModMenu_AT_Hip_FOV", UI = AliasMap.Slider, Text = "      Радиус FOV (1-100)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchHipFOV or 30 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchHipFOV = v return true end },
            { Key = "ModMenu_AT_Hip_Dist", UI = AliasMap.Slider, Text = "      Расстояние (1-500м)", ExpandHandle = "ModMenu_AT_Hip_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return math.floor((_G.LexusState.CustomTextData.AimTouchHipDist or 250) / 5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchHipDist = v * 5 return true end },

            -- AIMBOT SHOTGUN
            { Key = "ModMenu_AT_SG_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot Дробовик (Только Дробовик)", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.AimTouchSG end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSG = v return true end },
            { Key = "ModMenu_AT_SG_AutoFire", UI = AliasMap.Switcher, Text = "      Авто Огонь (нужно нажимать кнопку стрельбы для получения урона)", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSGAutoFire end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSGAutoFire = v return true end },
            { Key = "ModMenu_AT_SG_IgKnock", UI = AliasMap.Switcher, Text = "      Игнорировать Падение Врага", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSGIgKnock end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSGIgKnock = v return true end },
            { Key = "ModMenu_AT_SG_IgBot", UI = AliasMap.Switcher, Text = "      Игнорировать Ботов", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSGIgBot end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSGIgBot = v return true end },
            { Key = "ModMenu_AT_SG_Vis", UI = AliasMap.Switcher, Text = "      Проверка Стены (VisCheck)", ExpandHandle = "ModMenu_AT_SG_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSGVisCheck end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSGVisCheck = v return true end },
            { Key = "ModMenu_AT_SG_Prio", UI = AliasMap.Slider, Text = "      Приоритет (1:Центр 2:Близко 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGPrio or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchSGPrio = val return true end },
            { Key = "ModMenu_AT_SG_Bone", UI = AliasMap.Slider, Text = "      Позиция (1:Голова 2:Грудь 3:Живот 4:Бедра)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGBone or 2 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchSGBone = val return true end },
            { Key = "ModMenu_AT_SG_Cond", UI = AliasMap.Slider, Text = "      Условие (1:Наведение При Выстреле, 2:Постоянное Наведение)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGCond or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 2 then val = 2 end; _G.LexusState.CustomTextData.AimTouchSGCond = val return true end },
            { Key = "ModMenu_AT_SG_Spd", UI = AliasMap.Slider, Text = "      Плавность / Скорость (1-100)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGSpeed or 80 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSGSpeed = v return true end },
            { Key = "ModMenu_AT_SG_FOV", UI = AliasMap.Slider, Text = "      Радиус FOV (1-100)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGFOV or 40 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSGFOV = v return true end },
            { Key = "ModMenu_AT_SG_Dist", UI = AliasMap.Slider, Text = "      Расстояние (1-100м)", ExpandHandle = "ModMenu_AT_SG_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSGDist or 30 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSGDist = v return true end },
            
            -- SCOPE ALL (ПРИЦЕЛ ВСЕГДА)
            { Key = "ModMenu_AT_ScopeAll_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot С Прицелом (если смещение, отключите прицел и включите снова)", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.AimTouchScopeAll end, SetFunc = function(c,v) _G.LexusConfig.AimTouchScopeAll = v return true end },
            { Key = "ModMenu_AT_ScopeAll_IgKnock", UI = AliasMap.Switcher, Text = "      Игнорировать Падение Врага", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.LexusConfig.AimTouchScopeIgKnock end, SetFunc = function(c,v) _G.LexusConfig.AimTouchScopeIgKnock = v return true end },
            { Key = "ModMenu_AT_ScopeAll_IgBot", UI = AliasMap.Switcher, Text = "      Игнорировать Ботов", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.LexusConfig.AimTouchScopeIgBot end, SetFunc = function(c,v) _G.LexusConfig.AimTouchScopeIgBot = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Vis", UI = AliasMap.Switcher, Text = "      Проверка Стены (VisCheck)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", GetFunc = function() return _G.LexusConfig.AimTouchScopeVisCheck end, SetFunc = function(c,v) _G.LexusConfig.AimTouchScopeVisCheck = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Prio", UI = AliasMap.Slider, Text = "      Приоритет (1:Центр 2:Близко 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopePrio or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchScopePrio = val return true end },
            { Key = "ModMenu_AT_ScopeAll_Bone", UI = AliasMap.Slider, Text = "      Позиция (1:Голова 2:Грудь 3:Живот 4:Бедра)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopeBone or 2 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchScopeBone = val return true end },
            { Key = "ModMenu_AT_ScopeAll_Cond", UI = AliasMap.Slider, Text = "      Условие (1:Наведение При Выстреле, 2:Постоянное Наведение)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopeCond or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 2 then val = 2 end; _G.LexusState.CustomTextData.AimTouchScopeCond = val return true end },
            { Key = "ModMenu_AT_ScopeAll_Spd", UI = AliasMap.Slider, Text = "      Плавность / Скорость (1-100)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopeSpeed or 40 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchScopeSpeed = v return true end },
            { Key = "ModMenu_AT_ScopeAll_FOV", UI = AliasMap.Slider, Text = "      Радиус FOV (1-100)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopeFOV or 20 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchScopeFOV = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Dist", UI = AliasMap.Slider, Text = "      Расстояние (1-500м)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return math.floor((_G.LexusState.CustomTextData.AimTouchScopeDist or 300) / 5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchScopeDist = v * 5 return true end },
            { Key = "ModMenu_AT_ScopeAll_Pred", UI = AliasMap.Slider, Text = "      Предсказание Направления Движения", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopePred or 0 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchScopePred = v return true end },
            { Key = "ModMenu_AT_ScopeAll_Recoil", UI = AliasMap.Slider, Text = "      Автоматическая Компенсация Отдачи При Aim ( 3%-4% оптимально)", ExpandHandle = "ModMenu_AT_ScopeAll_Ex", MinValue = 0, MaxValue = 50, min = 0, max = 50, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchScopeRecoil or 0 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchScopeRecoil = v return true end },

            -- SCOPE SNIPE​R (СНajпЕРСКая)
            { Key = "ModMenu_AT_Sniper_Ex", UI = AliasMap.TitleSwitcher, Text = "   ▶ Aimbot С Прицелом (Снайперская)", ExpandHandle = "ModMenu_AT_Ex", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.AimTouchScopeSniper end, SetFunc = function(c,v) _G.LexusConfig.AimTouchScopeSniper = v return true end },
            { Key = "ModMenu_AT_Sniper_IgKnock", UI = AliasMap.Switcher, Text = "      Игнорировать Падение Врага", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSniperIgKnock end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSniperIgKnock = v return true end },
            { Key = "ModMenu_AT_Sniper_IgBot", UI = AliasMap.Switcher, Text = "      Игнорировать Ботов", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSniperIgBot end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSniperIgBot = v return true end },
            { Key = "ModMenu_AT_Sniper_Vis", UI = AliasMap.Switcher, Text = "      Проверка Стены (VisCheck)", ExpandHandle = "ModMenu_AT_Sniper_Ex", GetFunc = function() return _G.LexusConfig.AimTouchSniperVisCheck end, SetFunc = function(c,v) _G.LexusConfig.AimTouchSniperVisCheck = v return true end },
            { Key = "ModMenu_AT_Sniper_Prio", UI = AliasMap.Slider, Text = "      Приоритет (1:Центр 2:Близко 3:HP 4:%HP)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperPrio or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchSniperPrio = val return true end },
            { Key = "ModMenu_AT_Sniper_Bone", UI = AliasMap.Slider, Text = "      Позиция (1:Голова 2:Грудь 3:Живот 4:Бедра)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 4, min = 1, max = 4, Min = 1, Max = 4, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperBone or 1 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 4 then val = 4 end; _G.LexusState.CustomTextData.AimTouchSniperBone = val return true end },
            { Key = "ModMenu_AT_Sniper_Cond", UI = AliasMap.Slider, Text = "      Условие (1:Aim При Выстреле, 2:Постоянный Aim)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 2, min = 1, max = 2, Min = 1, Max = 2, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperCond or 2 end, SetFunc = function(c,v) local val = math.floor(v+0.5); if val < 1 then val = 1 end; if val > 2 then val = 2 end; _G.LexusState.CustomTextData.AimTouchSniperCond = val return true end },
            { Key = "ModMenu_AT_Sniper_Spd", UI = AliasMap.Slider, Text = "      Плавность / Скорость (1-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperSpeed or 30 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSniperSpeed = v return true end },
            { Key = "ModMenu_AT_Sniper_FOV", UI = AliasMap.Slider, Text = "      Радиус FOV (1-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperFOV or 20 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSniperFOV = v return true end },
            { Key = "ModMenu_AT_Sniper_Dist", UI = AliasMap.Slider, Text = "      Расстояние (1-500м)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return math.floor((_G.LexusState.CustomTextData.AimTouchSniperDist or 400) / 5) end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSniperDist = v * 5 return true end },
            { Key = "ModMenu_AT_Sniper_Pred", UI = AliasMap.Slider, Text = "      Предсказание Направления Движения (0-100)", ExpandHandle = "ModMenu_AT_Sniper_Ex", MinValue = 0, MaxValue = 100, min = 0, max = 100, GetFunc = function() return _G.LexusState.CustomTextData.AimTouchSniperPred or 0 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.AimTouchSniperPred = v return true end }
        }

        local StackSkin = {
            { Key = "ModMenu_ModEmote", UI = AliasMap.Switcher, Text = "Разблокировать Все VIP Действия (Эмоции) Открыть В Бою", GetFunc = function() return _G.LexusConfig.ModEmote end, SetFunc = function(c,v) _G.LexusConfig.ModEmote = v return true end },
            { Key = "ModMenu_ModSkin", UI = AliasMap.Switcher, Text = "Система Skin Мод Если Забанен Вкл HWID Если Еще Бан Выключить. ОТКРЫТЬ СУМОЧКУ В ИГРЕ ЧТОБЫ ВЫБРАТЬ", GetFunc = function() return _G.LexusConfig.ModSkin end, SetFunc = function(c,v) _G.LexusConfig.ModSkin = v return true end },
            { Key = "ModMenu_SkinDeadBox", UI = AliasMap.Switcher, Text = "Skin Мертвая Коробка (Зависит от скина оружия/авто) Очень Лагает На Слабых ПК Не Включать", GetFunc = function() return _G.LexusConfig.SkinDeadBox end, SetFunc = function(c,v) _G.LexusConfig.SkinDeadBox = v return true end },
            { Key = "ModMenu_SkinAttachment", UI = AliasMap.Switcher, Text = "Skin Аксессуары Оружия (Ствол, Рукоять, Празе) Отключит = Плавнее (вкл в бою скин будет на след бою)", GetFunc = function() return _G.LexusConfig.SkinAttachment end, SetFunc = function(c,v) _G.LexusConfig.SkinAttachment = v return true end },
            { Key = "ModMenu_KillMessage", UI = AliasMap.Switcher, Text = "Kill Messenger (VIP уведомления об убийстве) Выкл = Плавнее", GetFunc = function() return _G.LexusConfig.KillMessage end, SetFunc = function(c,v) _G.LexusConfig.KillMessage = v return true end },
            { Key = "ModMenu_KillCountUI", UI = AliasMap.Switcher, Text = "Счетчик Киллов (Показывать кол-во убийств по оружию) Выкл = Плавнее", GetFunc = function() return _G.LexusConfig.KillCountUI end, SetFunc = function(c,v) _G.LexusConfig.KillCountUI = v return true end },
            { Key = "ModMenu_SkinOpenLink", UI = AliasMap.Switcher, Text = "Skin Шляпа Нажать Уровень 1 Снять = Работает. Скин Рюкзак Экипировать Рюкзак 1 = Работает", GetFunc = function() return _G.LexusConfig.SkinOpenLink end, SetFunc = function(c,v) _G.LexusConfig.SkinOpenLink = v; if v == true then pcall(function() local Web = require("client.slua.logic.url.logic_webview_sdk"); if Web and Web.OpenURL then Web:OpenURL("https://t.me/braisconfig") end end) end return true end },
        }

        local StackCombat = {
            { Key = "ModMenu_FakeHWID", UI = AliasMap.Switcher, Text = "Фейк HWID (Защита от блокировки ID устройства)", GetFunc = function() return _G.LexusConfig.FakeHWID end, SetFunc = function(c,v) _G.LexusConfig.FakeHWID = v return true end },
            ...
            { Key = "ModMenu_Ipad_FOV", UI = AliasMap.Slider, Text = "   Угол FOV", ExpandHandle = "ModMenu_Ipad_Ex", MinValue = 1, MaxValue = 100, min = 1, max = 100, GetFunc = function() return (_G.LexusState.CustomTextData.IpadViewFOV or 120) - 90 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.IpadViewFOV = 90 + v return true end },

            { Key = "ModMenu_BugMan_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Растяжка Экрана (Бug Экрана/Большой Персонаж)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.BugManEnable end, SetFunc = function(c,v) _G.LexusConfig.BugManEnable = v return true end },
            { Key = "ModMenu_BugMan_Ratio", UI = AliasMap.Slider, Text = "   Степень Растяжки (Чем меньше = тем толще)", ExpandHandle = "ModMenu_BugMan_Ex", MinValue = 110, MaxValue = 200, min = 110, max = 200, GetFunc = function() return _G.LexusState.CustomTextData.BugManRatio or 133 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.BugManRatio = v return true end },

            { Key = "ModMenu_165FPS", UI = AliasMap.Switcher, Text = "Разблокировать 165 FPS ( выкл в этом бою, работает на след)", GetFunc = function() return _G.LexusConfig.UnlockFPS end, SetFunc = function(c,v) _G.LexusConfig.UnlockFPS = v; if v then _G.LexusState.GraphicsUnlocked = false end return true end },
            
            -- РАЗДЕЛЕНО WALL И ЦВЕТА ОТДЕЛЬНО
            { Key = "ModMenu_WallXuyenTuong", UI = AliasMap.Switcher, Text = "Wall сквозь Стены V1 (Только видеть сквозь)", GetFunc = function() return _G.LexusConfig.WallXuyenTuong end, SetFunc = function(c,v) _G.LexusConfig.WallXuyenTuong = v return true end },
            { Key = "ModMenu_ColorBodyV2", UI = AliasMap.Switcher, Text = "Раскраска Врага V2 (Базовый Chams)", GetFunc = function() return _G.LexusConfig.ColorBodyV2 end, SetFunc = function(c,v) _G.LexusConfig.ColorBodyV2 = v return true end },
            
            { Key = "ModMenu_ColorBodyV3_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ WALL V2 + ЦВЕТ V3 (Настройка Цвета & Контур)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.ColorBodyV3 end, SetFunc = function(c,v) _G.LexusConfig.ColorBodyV3 = v return true end },
            { Key = "ModMenu_V3_Hidden", UI = AliasMap.Slider, Text = "   Цвет За Стеной (1:Красный 2:Зеленый 3:Синий 4:Желтый 5:Фиолетовый 6:Белый)", ExpandHandle = "ModMenu_ColorBodyV3_Ex", MinValue = 1, MaxValue = 6, GetFunc = function() return _G.LexusState.CustomTextData.ColorV3Hidden or 1 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.ColorV3Hidden = v return true end },
            { Key = "ModMenu_V3_Vis", UI = AliasMap.Slider, Text = "   Цвет На Виду (1:Красный 2:Зеленый 3:Синий 4:Желтый 5:Фиолетовый 6:Белый)", ExpandHandle = "ModMenu_ColorBodyV3_Ex", MinValue = 1, MaxValue = 6, GetFunc = function() return _G.LexusState.CustomTextData.ColorV3Visible or 2 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.ColorV3Visible = v return true end },
            { Key = "ModMenu_V3_Thick", UI = AliasMap.Slider, Text = "   Толщина HDR Контур На Виду", ExpandHandle = "ModMenu_ColorBodyV3_Ex", MinValue = 1, MaxValue = 20, GetFunc = function() return _G.LexusState.CustomTextData.ColorV3Thickness or 4 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.ColorV3Thickness = v return true end },
            
            { Key = "ModMenu_WallVehicle", UI = AliasMap.Switcher, Text = "Wall Транспорт (Видеть Сквозь Стены)", GetFunc = function() return _G.LexusConfig.WallVehicle end, SetFunc = function(c,v) _G.LexusConfig.WallVehicle = v return true end },

            { Key = "ModMenu_WhiteBody", UI = AliasMap.Switcher, Text = "Белый Человек", GetFunc = function() return _G.LexusConfig.WhiteBody end, SetFunc = function(c,v) _G.LexusConfig.WhiteBody = v return true end },
            { Key = "ModMenu_BlackSky", UI = AliasMap.Switcher, Text = "Темное Небо (Black Sky)", GetFunc = function() return _G.LexusConfig.BlackSky end, SetFunc = function(c,v) _G.LexusConfig.BlackSky = v return true end },
            { Key = "ModMenu_RemoveFog", UI = AliasMap.Switcher, Text = "Убрать Туман ( выкл в этом бою, работает на след)", GetFunc = function() return _G.LexusConfig.RemoveFog end, SetFunc = function(c,v) _G.LexusConfig.RemoveFog = v return true end },
            { Key = "ModMenu_RemoveGrass", UI = AliasMap.Switcher, Text = "Убрать Траву ( выкл в этом бою, работает на след)", GetFunc = function() return _G.LexusConfig.RemoveGrass end, SetFunc = function(c,v) _G.LexusConfig.RemoveGrass = v return true end },
            { Key = "ModMenu_RemoveTrees", UI = AliasMap.Switcher, Text = "Убрать Деревья ( выкл в этом бою, работает на след)", GetFunc = function() return _G.LexusConfig.RemoveTrees end, SetFunc = function(c,v) _G.LexusConfig.RemoveTrees = v return true end },
            { Key = "ModMenu_WallClimb", UI = AliasMap.Switcher, Text = "Лезть на Стены", GetFunc = function() return _G.LexusConfig.WallClimb end, SetFunc = function(c,v) _G.LexusConfig.WallClimb = v return true end },
            { Key = "ModMenu_FastCar", UI = AliasMap.Switcher, Text = "Быстрая Машина Летающая", GetFunc = function() return _G.LexusConfig.FastCar end, SetFunc = function(c,v) _G.LexusConfig.FastCar = v return true end },

            -- [ДОБАВЛЕНО] МЕНЮ GLOW КОНТУР ОРУЖИЯ (Есть стрелка вниз)
            { Key = "ModMenu_WeaponGlow_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Glow Контур Оружия (HDR Свечение)", ExpandIndex = 0, GetFunc = function() return _G.LexusConfig.WeaponGlow end, SetFunc = function(c,v) _G.LexusConfig.WeaponGlow = v return true end },
            { Key = "ModMenu_WeaponGlowColor", UI = AliasMap.Slider, Text = "   Цвет Оружия (1:Красный 2:Зеленый 3:Синий 4:Желтый 5:Радуга)", ExpandHandle = "ModMenu_WeaponGlow_Ex", MinValue = 1, MaxValue = 5, GetFunc = function() return _G.LexusState.CustomTextData.WeaponGlowColor or 5 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.WeaponGlowColor = v return true end },
            { Key = "ModMenu_WeaponGlowThick", UI = AliasMap.Slider, Text = "   Толщина Контур Оружия", ExpandHandle = "ModMenu_WeaponGlow_Ex", MinValue = 1, MaxValue = 15, GetFunc = function() return _G.LexusState.CustomTextData.WeaponGlowThickness or 3 end, SetFunc = function(c,v) _G.LexusState.CustomTextData.WeaponGlowThickness = v return true end }
        }

        -- FIX ПЕРЕДАВАТЬ ID ЧИСЛО ВМЕСТО ПЕРЕДАВАТЬ ТЕКСТ И МЕНЯТЬ "loc" НА "Text"
        ...
        table.insert(SettingCatalog, 1, SettingPageDefine.ModMenu) -- Добавь 1 чтобы меню было первым
    end
    ...
    if UIManager and not UIManager._IsModMenuHooked then
        ...
        UIManager.ShowUI = function(config, ...)
            ...
            -- [ИСПР OШИБКА ПУСТОЙ НАСТРОЙКИ] - Вставить ТОЛЬКО VIP Меню в Principal Настр, ЗАБЛОКИР Пользоват UI
            if config and config.keyName then
                ...
                if string.find(lowerKeyName, "setting_main") and not string.find(lowerKeyName, "custom") then
                    ...
                    -- Проверить безоп args[1] массив Catalog Настр
                    if type(catalog) == "table" and catalog[1] and type(catalog[1]) == "table" and catalog[1].Key then
                        ...
                        if not hasModMenu then
                            table.insert(catalog, 1, SettingPageDefine.ModMenu) -- Добавь 1 чтобы меню было первым
                        end
                    end
                end
            end
...
local function ShowLexusVIPMenu() 
    ...
    pcall(function()
        ...
        local function Step_ScamAlert()
            Msg.Show(1, "ПРЕДУПРЕЖДЕНИЕ SCAM МОД", "Присоединиться ко мне в Telegram, чтобы избежать Мошенников Продажа Бесплатного Мода. tg @braisconfig\nДОБРО ПОЖАЛОВАТЬ ВСЕМ КОТОРЫЕ КРАДУТ МОД ДОНЬ ДОБРО ПОЖАЛОВАТЬ ЭТОТ ПУЧОК БЛИН, У МЕНЯ ТОЛЬКО 1 TELE АКАУНТ БУДЬТЕ ОСТОРОЖНЫ", function() local Web = require("client.slua.logic.url.logic_webview_sdk"); if Web and Web.OpenURL then Web:OpenURL("https://t.me/braisconfig") end end, function() end, "ПРИСОЕДИНИТЬСЯ", "ЗАКРЫТЬ")
            _G.LexusState.MenuStep = 99
            _G.LexusMenuAlreadyShown = true
        end

        local function Step_Welcome()
            Msg.Show(1, "ДОБРО ПОЖАЛОВАТЬ", "Привет Я Донг. Вам не нужно использовать набор или конфигурации, теперь есть VIP МЕНЮ в Настройках игры! \n НО СЛУШАЙТЕ, ИСПОЛЬЗУЙТЕ МАЛО ФУНКЦИЙ, ОЧЕНЬ ЛАГОВО ПОНЯЛИ, Я БОЮСЬ, ВАШ ПК НЕ МОЖЕТ ПЕРЕНОСИТЬ, И НЕ ПОКАЗЫВАТЬ(PK) ИГРАЙТЕ ТЕХНИЧНО ИНАЧЕ БАН", 
            function() 
                _G.InitModMenuTab()
                Notify("ДОБАВЛЕНО 'VIP МОД МЕНЮ' В НАСТРОЙКИ ИГРЫ!\nОткройте Настройки (Шестеренка) -> VIP МОД МЕНЮ Чтобы Вкл/Выкл И Настроить В Игре!")
                Step_ScamAlert()
            end, 
            function() end, "ОТКРЫТЬ МЕНЮ В ИГРЕ", "ЗАКРЫТЬ")
        end
...
-- ========================================== 
-- LOGIC РАЗБЛОКИРОВК 165 FPS И UI IPAD VIEW 
-- ========================================== 
...
-- ========================================== 
-- ИНИЦИАЛИЗАЦИЯ ESP СИСТЕМЫ (ОСНОВНАЯ)
-- ========================================== 
local function InitializeNativeESP() 
    ...
    pcall(function() 
        ...
        local function ApplyCfg(cfg)
            ...
            -- [FIX ESP ТИП 4] Вместо использования 1003 который игра может удалить, создаем эксклюзивный ID 8888
            cfg[8888] = { 
                ...
                bBindOutScreen = true,
                bBindBlocked = true, 
                bIsBindingActor = true,     -- Обязательно нужен чтобы следовать за врагом
                BindSocketName = "head",
                bUseLuaWorldSocketName = true, 
                WorldPositionOffset = FVector(0, 0, 30),
                bNeedPreLoad = true,        -- Обязательно чтобы предзагрузить UI (предотвратить ошибку)
                Priority = 2 
            } 
            cfg[9999] = { 
                UIPathName = "/Game/Mod/EvoBase/BluePrints/UIBP/QuickSign/QuickSign_TipHitEnemy_UIBP_New.QuickSign_TipHitEnemy_UIBP_New_C",
...
-- ========================================== 
-- СТЕНА ФУНКЦИЯ & ГЛОБАЛЬНОЕ ВОССТАНОВЛЕНИЕ
-- ==========================================
...
local function ApplyColorBodyV2(enemy, pc, markData)
    pcall(function()
        ...
        -- [FIX ПРОТИВ СИЛЬНЫЙ LAG МНОГО ЛЮДЕЙ]: Ограничить Raycast Check Стены 0.3s один раз
        -- Избежать стрельбы тысяч физических лучей каждую секунду, сжигая CPU
        local curTime = os.clock()
        ...
        -- Если нет изменения цвета / количество одежды то сразу прервать, экономить процессор
        if not isMeshChanged and markData.LastHiddenState == hidden and markData.LastColorHash == colorHash then return end
        
        -- [ИСПРАВЛЕНИЕ RAM]: Удалить старые Material мусор когда враг меняет оружие/броню чтобы избежать мусор VRAM
                ...
                for i = 0, 10 do
                    ...
                    if Valid(baseMat) then
                        ...
                        if string.find(matName, "Master_Mask", 1, true) then
                            ...
                            -- [FIX МЁРТВАЯ ПАМЯТЬ]: Вместо использования tostring(mesh) создание мусорной строки, использовать локальный индекс
...
-- ==========================================
-- ФУНКЦИЯ ЦВЕТА V3 (РАЗДЕЛЬНЫЙ КОД ОТ ВАШЕГО ИСХОДНОГО - РАБОТАЕТ С Z-BUFFER)
-- [УЖЕ ИСПРАВЛЕН ОШИБКА ПОТЕРЯ ЦВЕТА ПРИ СМЕНЕ LOD & ОПТИМИЗАЦИЯ ПРОТИВ DROP FPS МНОГО ЛЮДЕЙ]
-- ==========================================
local function ApplyColorBodyV3(enemy, markData)
    pcall(function()
        ...
        -- Создать хэш код для обнаружения изменение цвета/толщины ползунка
        ...
        local function GetColorRGB(choice)
            if choice == 1 then return 255, 0, 0 end -- Красный
            if choice == 2 then return 0, 255, 0 end -- Зеленый
            if choice == 3 then return 0, 0, 255 end -- Синий
            if choice == 4 then return 255, 255, 0 end -- Желтый
            if choice == 5 then return 255, 0, 255 end -- Фиолетовый/Розовый
            if choice == 6 then return 255, 255, 255 end -- Белый
            return 255, 0, 0 -- По умолчанию красный
        end

        local hR, hG, hB = GetColorRGB(hidChoice)
        local vR, vG, vB = GetColorRGB(visChoice)

        -- Цвет За Стеной (invisColor)
        local invisColor = { R=hR, G=hG, B=hB, A=255, r=hR, g=hG, b=hB, a=255 }
        
        -- Цвет Контур Виден HDR (visColor)
        local glowIntensity = 80.0 
        local LinearColorClass = import("LinearColor") or _G.FLinearColor
        ...
        for meshIndex, comp in ipairs(meshes) do
            if Valid(comp) then
                ...
                for i = 0, 10 do
                    ...
                    -- Если не MID или человек меняет ползунок цвета -> Обновить снова
...
local function UndoColorBodyV3(enemy, markData)
    pcall(function()
        if markData.ColorV3Applied then
            ...
            markData.LastMeshCountV3 = 0 -- Сбросить Личный Счет чтобы возможно включить снова
...
-- ========================================== 
-- СИСТЕМА AIMBOT V2 НОВАЯ ИНТЕГРАЦИЯ (UPDATE KISMET SMOOTH)
-- ========================================== 
...
_G.AimTouch = function()
    pcall(function()
        ...
        -- Логика выстрела Если Цель Потеряна / Враг Мёртв Или Дробовик Без Патрона
        ...
        -- Дробник Без Патрона Прекратить Aim Чтобы Игра Перезарядить
        ...
        -- Логика дополнения: Предсказание и Компенсация Отдачи
        local predVal = 0 
        local recoilCompVal = 0 

        -- КЛАССИФКАЦИЯ КОНФИГУРАЦИИ ПО ТЕКУЩЕМУ СОСТОЯНИЮ
        ...
        elseif isADS then
            if isSniper and _G.LexusConfig.AimTouchScopeSniper then
                ...
                predVal = _G.LexusState.CustomTextData.AimTouchSniperPred or 0 -- Получить значение Предсказание Снайпер
            elseif _G.LexusConfig.AimTouchScopeAll then
                ...
                predVal = _G.LexusState.CustomTextData.AimTouchScopePred or 0 -- Получить значение Предсказание Обычного оружия
                recoilCompVal = _G.LexusState.CustomTextData.AimTouchScopeRecoil or 0 -- Получить значение компенсации Отдачи
            else
                return
            end
        else
        ...
        for i, target in ipairs(enemies) do
            ...
            -- [FIX ПАДЕНИЕ FPS]: Заблокировать Raycast check стен, проверять 0.2s один раз (Достаточно плавный без перегрузки CPU)
            if useVisCheck then
                local curTime = os.clock()
        ...
        -- LOGIC 1: ПРЕДСКАЗАНИЕ (ПРЕДСКАЗАТЬ НАПРАВЛЕНИЕ БЕГА)
        if predVal > 0 then
            pcall(function()
                local tVelocity = nil
                -- Unreal Engine Взять вектор движения врага
                ...
                if tVelocity and (tVelocity.X ~= 0 or tVelocity.Y ~= 0) then
                    local distToEnemy = player:GetDistanceTo(bestTarget) / 100.0 -- Расстояние метры
                    
                    -- Расчет времени полета пули (Time-Of-Flight) пропорционально расстоянию и переменной передачи
                    -- Коэффициент 800.0 представляет скорость падения пули моделирование, 50.0 это средний ползунок
                    local ToF = (distToEnemy / 800.0) * (predVal / 50.0) 
                    
                    -- Переместить координату Aim впереди направления бега
        ...
        -- [НАЧАТЬ FIX] Компенсировать разницу Камеры при открытии прицела (ADS) чтобы не было смещение
        if isADS then
            local camRot = nil
        ...
        -- LOGIC 2: КОМПЕНСАЦИЯ ОТДАЧИ (ЭП ЦЕНТР / КОМПЕНСАЦИЯ ОТДНО ИЗБЕЖАТЬ СТРЕЛЯТЬ ЧЕРЕЗ ГЛАВУ)
        -- Только нажимать, когда оружие стреляет и значение Recoil > 0 (Использовать для обычно Оружие)
        if recoilCompVal > 0 and isFiring then
            -- В UE4, тянуть Pitch вниз (меньше) равнозначно с действиc вкладке экрана вниз
            -- Ползунок recoilCompVal (0-50), каждый кадр компенсировать одно количество зависит от степени Отдачи
            local pullDownForce = (recoilCompVal / 50.0) * 1.5 -- Настрой коэффициент 1.5 произвольно чтобы нажимать сильнее
...
-- ========================================== 
-- СИСТЕМА WALL & ESP ПРЕДМЕТОВ/ТРАНСПОРТА СУПЕР ПЛАВНАЯ (ОПТИМИЗАЦИЯ ПОД 70M)
-- ========================================== 
local ItemDatabase = {
    -- AR
    ...
    -- Пулемет (Объединено с AR для компактности)
...
-- ========================================== 
-- СИСТЕМА WALL & ESP ПРЕДМЕТОВ/ТРАНСПОРТА СУПЕР ПЛАВНАЯ (ПОЛНАЯ 100% ОСНОВАНИЕ)
-- ========================================== 
...
_G.RunOptimizedItemAndVehicleESP = function(pc)
    ...
    -- 1. СКАНИРОВАНИЕ ACTOR И ОБРАБОТКА ФИЗИКИ 1.0 СЕК / РАЗ (ПРОТИВ ПАДЕНИЯ FPS ПРИ ПОДБОРЕ ПРЕДМЕТОВ)
    if curTime - _G.LastScanItemTime > 1.0 then
        _G.LastScanItemTime = curTime
        local player = GameplayData.GetPlayerCharacter()
        if not slua.isValid(player) then return end

        -- Обработка WALL Транспорт (Сохранить даль​​ьность взгляда 200m)
        ...
        -- ОБРАБОТКА ESP И CHAMS ПРЕДМЕТЫ (Позиционирование текста и Glow под 70m)
                        ...
                        if slua.isValid(item) then
                            ...
                            if dist <= 7000 then
                                ...
                                if itemData then
                                    ...
                                    -- Обрабатывать массив и рисовать Glow только при включении
                                    if isShow then
    ...
    -- 2. РИСОВАНИЕ НАЗВАНИЯ ПРЕДМЕТА НЕПРЕРЫВНО В КАДРЕ(Очень легкий, выполняется каждый кадр)
...
-- ========================================== 
-- ГЛАВНЫЙ ЦИКЛ (MAIN LOOP) МАКСИМАЛЬНАЯ ОПТИМИЗАЦИЯ
-- ========================================== 
local function MainLoop() 
    if isExpired then return end

    -- =====================================================================
    -- СИСТЕМА ПОЛУЧЕНИЯ HWID ОРИГИНАЛ & СМЕНА FWID ВИРТУАЛЬНО (подделыватель) ПРОТИВ БАН
    -- =====================================================================
    pcall(function()
        local SystemLib = import("KismetSystemLibrary")
        if SystemLib and not _G.FakeHWID_Hooked then
            -- Сохранить исходную функцию получения HWID
            _G.Original_GetDeviceId = SystemLib.GetDeviceId

            -- Переграфить функцию игры
            SystemLib.GetDeviceId = function(...)
                if _G.LexusConfig.FakeHWID then
                    if not _G.FakeHWID_String then
                        -- Создать случайный виртуальный HWID 32 символа
                    ...
                    -- Вернуть виртуальный HWID
                    return _G.FakeHWID_String
                end
                
                -- Если выключен FAKE HWID то вернуть настоящий HWID
    ...
    -- Самостоятельная функция чтобы получить первоначальный HWID (если нужно отображать в будущем)
    ...
    -- ОЧИСТКА ВСЕГО МУСОРА ИЗ ПАМЯТИ RAM ПРИ СМЕРТИ, СМЕНЕ КАРТЫ, ВХОДЕ В ЛОББИ
    if not Valid(localPlayer) then 
        ...
        -- Очистить объекты UE4 MIDs для максимального освобождения RAM между матчами
    ...
    -- [ДОБАВЛЕНО] ЛОГИКА GLOW ОРУЖИЯ (НЕЗАВИСИМО & СУПЕР ПЛАВНО 0.5s/Lần - ГАРАНТИЯ 0% DROP FPS)
    ...
    -- ========================================================
    -- ЛОГИКА КОМПЕНСАЦИИ ОТДАЧИ (ЦЕНТРОВКА) ТОЛЬКО ТОЛЬКО ДЛЯ ОСНОВНОЙ AIMBOT (УЖЕ FIX LAG ĐÔNG ЧЕЛОВЕК)
    -- ========================================================
    pcall(function()
        if _G.LexusConfig.CustomAimbot and localPlayer.bIsWeaponFiring and localPlayer.bIsGunADS then
            ...
            if outerRecoilVal > 0 then
                ...
                -- [ИСПРАВЛЕНИЕ НАГРУЗКИ CPU]: Quét mục tiêu 0.2s/раз вместо 100 раз/сек чтобы избежать quá tải máy когда check FOV
    ...
    -- ========================================================
    -- THỰC THI MOD SKIN ПОЛУЧИТЬ ИНТЕГРИРОВАН ПРЯМО В MAIN LOOP (TỐI ƯU TUYỆT ДЛЯ)
    -- ========================================================
    -- ========================================================
    -- THỰC THI MOD SKIN ТЕЛО / PET / KILL MESSAGE / Принудительно запустить V7.5
    -- ========================================================
    if _G.LexusConfig.ModSkin then
        local curTime = os.clock()
        -- Увеличено время проверки с 1.0s до 2.5s чтобы избежать лагов при переключении
            ...
            pcall(function()
                ...
                if isAlive then
                    ...
                    if _G.AddOutfit then
                        -- [ЧЕТКОЕ РАЗДЕЛЕНИЕ СПЯЩИХ ПОТОКОВ]
                        if _G.AddOutfit.isInRealMatch() then
                            -- ПОТОК 1: В МАТЧЕ (ЛОББИ СПЯЩИЙ)
                            _G.AddOutfitLobbyRestored = false 
                            
                            -- [FIX FPS] РАЗДЕЛЕН ПРОЦЕСС ЗАГРУЗКИ СКИНОВ (STAGGERED LOADING)
    ...
    -- CHẶN HIGGSBOSON THEO THỜI GIAN THỰC LÀM AN TOÀN TUYỆT ДЛЯ MÀ НЕТ GÂY VĂNG GAME
    pcall(function()
        if Valid(pc) then
    ...
    -- ВОССТАНОВЛЕНИЕ И НАСТРОЙКА AIMBOT HEAD COMPONENT МГНОВЕННО ВКЛ/ВЫКЛ
    pcall(function()
    ...
    -- ВОССТАНОВИТЬ ГРАФИКУ СРАЗУ ЕСЛИ ОТКЛ (TẮT LÀ TẮT LIỀN)
    local now = os.clock()
    pcall(function()
    ...
    pcall(function()
        ...
        if runInject_Global then
            ...
            -- КОГДА MAGIC BULLET ОТКЛЮЧЕН, ВОССТАНОВИТЬ HASH В 0
    ...
    pcall(function()
        ...
        for key, data in pairs(_G.LexusState.EnemyMarks) do
            if not currentValidKeys[key] then
                ...
                -- [ИСПРАВЛЕНИЕ RAM]: Очистка мусора AimTouch VisCheck от мертвых или слишком далеких врагов
        ...
        for _, enemy in pairs(allCharacters) do
            if Valid(enemy) and enemy ~= localPlayer and enemy.TeamID ~= localPlayer.TeamID then
                ...
                if not bIsReallyDead then
                    -- [FIX LỖI ПОТЕРЯ ЗДОРОВЬЯ КОГДА ПРЫЖОК НА ПАРАШЮТЕ/ВОСКРЕШЕНИЕ]: Проверка смотреть враг есть быть изменить Actor (персонаж новый) нет.
                    -- если есть, удаление весь Marker (UI) быть kẹt ở xác старый чтобы code bên ниже рисование обратно вверх персонаж новый.
                    if markData.lastEnemyActor ~= enemy then
                        if markData.hpMark then SafeRemoveMark(markData.hpMark); markData.hpMark = nil end
                        if markData.hpMark8 then SafeRemoveMark(markData.hpMark8); markData.hpMark8 = nil end -- Xóa сразу мусор от ESP 8
                    ...
                    -- МАКСИМАЛЬНО ОПТИМИЗИРОВАНО: Chỉ Apply когда thật sự cần
                    ...
                    -- МАКСИМАЛЬНО ОПТИМИЗИРОВАНО
                    if _G.LexusConfig.ColorBodyV2 then 
                        -- В ЭТОЙ ФУНКЦИИ ОГРАНИЧЕН PC:LINEOFSIGHTTO ДЛЯ ИЗБЕЖВАНИЯ ПЕРЕГРУЗКИ CPU
                        ApplyColorBodyV2(enemy, pc, markData) 
                    else
                        UndoColorBodyV2(enemy, markData)
                    end
                    
                    -- CHỨC NĂNG ЦВЕТ V3 (LỘ DIỆN XANH LÁ + SAU TƯỜNG MÀU ĐỎ) ОЧЕНЬ ỔN ĐỊNH
                    ...
                    -- BUG MAN: Растяжение врага увеличивает хитбокс (FAT BODY) - УЖЕ TỐI ƯU
                    pcall(function()
                        if Valid(eMesh) then
                            ...
                            if _G.LexusConfig.BugManEnable and _G.LexusState.CustomTextData then
                                ...
                                if targetScale > 2.0 then targetScale = 2.0 end -- ЗАЩИТА ОТ ГРАФИЧЕСКИХ ОШИБОК ПРИ ПЕРЕРАСТЯЖКЕ
                            end
                            
                            -- [ИСПРАВЛЕНИЕ МУСОРА RAM]: РАСШИРЕНИЕ КОСТЕЙ ТОЛЬКО ПРИ ИЗМЕНЕНИИ (Bật/tắt или kéo thanh trượt)
                    ...
                    -- ЛОГИКА MAGIC BULLET (УЖЕ FIX LAG ĐÔNG ЧЕЛОВЕК С ПОМОЩЬЮ UNIQUE ID)
                    pcall(function()
                        local EnemyMesh = eMesh
                        if slua.isValid(EnemyMesh) then
                            -- [ИСПРАВЛЕНИЕ НАГРУЗКИ CPU]: ИСПОЛЬЗОВАТЬ НАСТОЯЩИЙ ID ПЕРСОНАЖА. Không использовать tostring() потому что SLUA tự удаление/tạo обратно chuỗi liên tục
                            -- gây lỗi tính toán обратно 50 кость lặp đi lặp обратно когда đông người.
                            local uniqueID = type(enemy.GetUniqueID) == "function" and enemy:GetUniqueID() or tostring(enemy.PlayerKey or enemy)
                            
                            -- РАСЧЕТ КОСТЕЙ ТОЛЬКО 1 РАЗ cho mỗi kẻ враг (trừ когда bạn kéo thanh толькоnh size)
                            ...
                            if slua.isValid(PhysicsAsset) and PhysicsAsset.SkeletalBodySetups then
                                ...
                                -- ОПТИМИЗАЦИЯ УРОВНЯ 2: если скелет này уже từng được phóng to bởi một kẻ враг khác, использовать сразу, нет запуск цикл
                                ...
                                markData.MagicTargetID = uniqueID -- СОХРАНИТЬ СТАТИЧЕСКИЙ ID
                            end
                        end
                    end)
                    ...
                    if _G.LexusConfig.EspLoai6 then
                        pcall(function()
                            local curTime = os.clock()
                            -- МАКСИМАЛЬНАЯ ОПТИМИЗАЦИЯ 1: БЛОКИРОВКА РИСОВАНИЯ HUD 20 FPS (0.05s/раз) вместо 100 FPS
                            -- Game все равно mượt, но CPU нет быть гореть потому что спам команда AddDebugText
                            if markData.LastEsp6Time == nil or (curTime - markData.LastEsp6Time) >= 0.05 then
                                ...
                                if Valid(MyHUD) and Valid(eMesh) and aLoc then
                                    if distM <= 250 then
                                        -- ПОЛУЧИТЬ КООРДИНАТЫ ГОЛОВЫ, если нет есть функция này тогда пропустить
                                        if type(eMesh.GetSocketLocation) == "function" then
                                            for _, bName in ipairs(GLOBAL_BONE_LIST) do
                                                
                                                -- МАКСИМАЛЬНАЯ ОПТИМИЗАЦИЯ 2: Враг дальше 50м рисовать только Голову, Шею, Бедра. Пропустить руки ноги поддержка мусор
                                                if distM > 50 and (bName ~= "head" and bName ~= "pelvis" and bName ~= "neck_01") then
                                                    -- НЕ РИСОВАТЬ РУКИ/НОГИ НА РАССТОЯНИИ
                                                else
                                                    local wLoc = eMesh:GetSocketLocation(bName)
                                                    if wLoc then
                                                        -- РАСЧЕТ ТОЧНОГО OFFSET ДЛЯ HUD
                                                        ...
                                                        -- РИСОВАТЬ ЯКОРЬ СУСТАВА (Время жизни 0.06s для плавного соединения с кадром 0.05s)
                                                        MyHUD:AddDebugText(mark, enemy, 0.06, offset, offset, color, true, false, true, nil, fixedSize, true)
                                                    end
                                                end
                                            end
                                        end
                                        -- ВНИМАНИЕ: ПОЛНОСТЬЮ УДАЛЕНА ФУНКЦИЯ РИСОВАНИЯ СВЯЗЕЙ (GLOBAL_CONNECTIONS)
                                        -- Vì использовать точка "." формирующая линию является ГЛАВНАЯ ПРИЧИНА ПАДЕНИЯ FPS 
                                    end
                                end
                            end
                        end)
                            ...
                            if Valid(MyHUD) then
                                ...
                                if distM <= 400 then
                                    ...
                                    if _G.LexusConfig.Esp7_TuThe then
                                        ...
                                        if pose == 0 or pose == "Stand" then stateText = "Стоит"
                                        elseif pose == 1 or pose == "Crouch" then stateText = "Сидит"
                                        elseif pose == 2 or pose == "Prone" then stateText = "Лежит"
                                        else stateText = "Стоит" end
                                    end
                                    ...
                                    if _G.LexusConfig.Esp7_VuKhi then
                                        ...
                                        if stateText ~= "" then
                                            stateText = stateText .. " - " .. (markData.AK_CACHED_WEP_NAME or "Без оружия")
                                        else
                                            stateText = (markData.AK_CACHED_WEP_NAME or "Без оружия")
                                        end
                                    end

                                    -- 3. Рисовать на экране если включено 1 trong 2
                                    if stateText ~= "" then
                    ...
                    -- МАКСИМАЛЬНО ОПТИМИЗИРОВАНО: SETVISIBILITY ДЛЯ UI ЗДОРОВЬЯ ТОЛЬКО ПРИ НЕОБХОДИМОСТИ
                    if showFrameUI then
                        pcall(function()
                    ...
                    -- [ESP ТИП 1 (уже Fix Lỗi)]: Оставить thanh máu (hpMark) и расстояние (distMark)
                    ...
                    -- [НЕЗАВИСИМЫЙ ESP ТИП 8 (уже Fix Lỗi)]: Копировать логику thanh máu ESP 1, но запуск переменную hpMark8 отдельно
                    ...
                    if _G.LexusConfig.EspRadar then
                        -- ИСПРАВЛЕНИЕ ОШИБКИ ПЕРЕМЕННЫХ (nil/false/0) и gọi ID 8888 монопольно
                    ...
                    -- [ESP КОНТУР - Y CHANG 100% LOGIC ОБНАЖЕНИЕ V3]: Свечение с настройкой HDR
                    if _G.LexusConfig.EspOutline then
                        pcall(function()
                            ...
                            if markData.OutlineState ~= outlineHash or markData.LastMeshCountOutline ~= currentMeshCount then
                                ...
                                local r, g, b = 255, 255, 0 -- Желтый (По умолчанию)
                                ...
                                for _, comp in ipairs(meshes) do
                                    if Valid(comp) then
                                        -- ОБЯЗАТЕЛЬНО КАК V3: Принудительная модель затенения для активации HDR (Bloom)
                                        pcall(function()
                                            comp.UseScopeDistanceCulling = false 
                                            comp.PrimitiveShadingStrategy = 1
                                            comp.ShadingRate = 6
                                        ...
                                        -- Y CHANG V3: Рисовать Outline đè вверх сверху с помощью функция исходной от Engine
                                        if comp.SetDrawIdeaOutline then
                                            comp:SetDrawIdeaOutline(true)
                                            ...
                                            if comp.OverrideIdeaOutlineThickness then
                                                -- Размер контура следует за слайдером в Меню
                                ...
                                markData.LastMeshCountOutline = currentMeshCount -- Сохранить количество аксессуаров
                            end
                        end)
                    else
                        pcall(function()
                            if markData.OutlineState ~= "OFF" then
                                ...
                                for _, comp in ipairs(meshes) do
                                    if Valid(comp) then
                                        -- Восстановить модель затенения по умолчанию при выключении
                                        pcall(function()
                                            comp.PrimitiveShadingStrategy = 0
                                            comp.ShadingRate = 1
                                        end)
                    ...
                    if not markData.IsCleanedUp then
                        ...
                        markData.hpMark = nil
                        SafeRemoveMark(markData.hpMark8) -- Очистка ESP 8
        ...
        if _G.LexusConfig.EspLoai7 and _G.LexusConfig.Esp7_SoLuong then
            pcall(function()
                ...
                if Valid(MyHUD) then
                    ...
                    local text = string.format("Враги вокруг: %d", totalEnemies)
        ...
        -- ==========================================================
        -- [LOGIC ESP БОМБА VVIP] - OPTIMIZED WITH WEAK CACHE (100% GỐC, НЕТ LAG)
        -- ==========================================================
        if _G.LexusConfig.EspBomMaster and (_G.LexusConfig.EspItemBom or _G.LexusConfig.EspActiveBom) then
            pcall(function()
                ...
                if Valid(MyHUD) then
                    ...
                    -- Инициализация кэша с Weak Table для автоматической очистки
                    ...
                    if gameInstance and _G.CachedGameplayStatics then
                        ...
                        -- ТЯЖЕЛЫЙ СКАНИРУЮЩИЙ ПОТОК: Запускать 0.5s/раз вместо каждого кадра
                            ...
                            if allActors then
                                for _, actor in pairs(allActors) do
                                    if slua.isValid(actor) and not actor.bHidden and not actor.bTearOff then
                                        ...
                                        -- 1. ПРОВЕРКА КЭША (CACHE) SIÊU TỐC
                                        -- Если этот актор уже từng сканировать и НЕТ PHẢI BOM -> Пропустить lập tức (Снижение 99% Lag)
                                        if not _G.NonBombCache[actor] then
                                            ...
                                            if isKnownBomb then
                                                ...
                                                -- Первое появление этого актора, tiến hành проверка имя (Rất ít когда xảy ra)
                                                local nameLower = nil
                                            ...
                                            -- если является Bom валидная (từ Cache или только найдена)
                                            if bType > 0 then
                                                ...
                                                if not isPendingKill then
                                                    ...
                                                    -- Удалить из кэша если бомба взорвана/переменную mất
                            ...
                            for _, item in ipairs(bombList) do
                                ...
                                if slua.isValid(bomb) and not bomb.bHidden then
                                    ...
                                    if distM > 0 and distM <= maxDist then
                                        ...
                                        elseif bType == 3 then displayName = "ОГОНЬ"; bombColor = isItem and {R=255, G=160, B=50, A=255} or {R=255, G=100, B=0, A=255}
                                        elseif bType == 4 then displayName = "ОСЛЕПЛЕНИЕ"; bombColor = isItem and {R=150, G=255, B=255, A=255} or C_CYAN
                                        elseif bType == 5 then displayName = "ДЫМОВАЯ"; bombColor = isItem and {R=150, G=255, B=150, A=255} or {R=100, G=255, B=100, A=255} end
        ...
        -- ==========================================================
        -- [LOGIC ESP ТРАНСПОРТ - VEHICLE ESP VVIP] - OPTIMIZED
        -- ==========================================================
        -- ==========================================================
        -- [LOGIC ESP ТРАНСПОРТ - VEHICLE ESP VVIP] - OPTIMIZED БЕЗ ЗДОРОВЬЯ (SIÊU NHẸ)
        -- ==========================================================
            ...
            pcall(function()
                ...
                if Valid(MyHUD) then
                    ...
                    if gameInstance and _G.CachedGameplayStatics then
                        ...
                        -- ОСНОВНОЙ СКАНИРУЮЩИЙ ПОТОК: 1.0s сканировать 1 раз.
                                ...
                                for _, veh in pairs(allVehicles) do
                                    if slua.isValid(veh) and not veh.bHidden and not veh.bTearOff then
                                        ...
                                        if not isPendingKill then
                                            ...
                                            elseif string.find(nameLower, "glider") then displayName = "Планер"
    ...
    if not _G.LexusNotifiedPopup then
        pcall(function()
            ...
            if Msg and Msg.Show then
                Msg.Show(1, "МОД ИСТЕК", "ВАША ВЕРСИЯ МОДА ИСТЕКЛА!\nПОЖАЛУЙСТА НАПИШИТЕ АДМИНУ ДЛЯ ПРОДЛЕНИЯ.\ntg @braisconfig", 
                function() 
                    local Web = require("client.slua.logic.url.logic_webview_sdk")
                    if Web and Web.OpenURL then Web:OpenURL("https://t.me/braisconfig") end 
                end, 
                function() end, "НАПИСАТЬ АВТОРУ МОДА", "ЗАКРЫТЬ")
...
local function FastTick() 
    if isExpired then 
        if not _G.LexusNotifiedExpire then
            Notify("МОД ИСТЕК! ПОЖАЛУЙСТА НАПИШИТЕ АДМИНУ ДЛЯ ПРОДЛЕНИЯ.\ntg @braisconfig")
...
if not isExpired then
    FastTick() 
    Notify("Вы используете Mod VVIP 4 | tg @braisconfig")
else
    FastTick() 
end
...
-- ==============================================================================
-- ================= НАЧАЛО CORE ADD-OUTFIT V7.5 (СИСТЕМА СКИНОВ) =================
-- ==============================================================================
-- Таблица отображения ID аксессуаров на индекс массива
...
-- ВСТАВЬТЕ ID ВАШИХ АКСЕССУАРОВ В Фигурные скобки ниже ↓↓↓
_G.VIP_Attachments = {
...
-- ВСТАВЬТЕ ID ВАШИХ АКСЕССУАРОВ ВЫШЕ ↑↑↑
...
_G.DeadBox_TemperRequest = function(PlayerController)
    ...
    -- Оптимизация: получить ID игрока заранее и ID súng/xe ở ngoài цикл чтобы избежать tính toán обратно
    ...
    pcall(function()
        ...
        if slua.isValid(curVeh) and _G.CurrentEquipVehicleID and _G.CurrentEquipVehicleID ~= 0 then
            ...
            -- [FIX CHUẨN VIP]: Lấy ID от vũ khí đang cầm сверху tay для правильного отображения hòm xác, Bỏ цикл чтобы против Drop FPS
            ...
            if slua.isValid(curWeapon) then
                ...
                -- Đối chiếu với kho Skin уже сохранение чтобы получить правильный ID Skin текущий
    ...
    for i = 1, count do
        ...
        if slua.isValid(deadBoxActor) and not deadBoxActor.bIsTDSkinApplied then
            ...
            -- So sánh сверхбыстрое с помощью MyPlayerключ уже cache
...
--[[ AddOutfit v7.5 — Tích hợp система выбора Skin через гардероб (Wardrobe) ]]
local F = {}
local DEBUG = false  
function F.log(...)
...
-- Bảng ID суперкаров (Добавлять свободно если есть ID новый)
local ITEMS = {
    -- ==============================================================================
    -- ОСНОВНАЯ СИСТЕМА V7.5 (НЕТ ПОЛУЧИТЬ XÓA DÒNG NÀY)
    -- ==============================================================================
    ...
    -- ==============================================================================
    -- 1. ОБНОВЛЕННОЕ ОРУЖИЕ (БРАТЬ ТОЛЬКО НАИВЫСШИЙ УРОВЕНЬ КАЖДОГО ОРУЖИЯ)
    -- ==============================================================================
    -- [ M416 ]
    1101004163, -- Hoàng Gia Lộng Lẫy - M416 (Уровень 8)
    1101004201, -- Bạch Lân Nhả Ngọc - M416 (Уровень 8)
    1101004209, -- Thủy Triều Dậy Sóng - M416 (Уровень 8)
    1101004218, -- Ma Ảnh - M416 (Уровень 8)
    1101004226, -- Phong Ấn U Minh - M416 (Уровень 8)
    1101004236, -- Синий Sư Đoạt Mệnh - M416 (Уровень 8)
    1101004246, -- Hỏa Liên - M416 (Уровень 8)
    1101004046, -- Băng giá - M416 (Уровень 7)
    1101004062, -- Chú hề - M416 (Уровень 7)
    1101004078, -- Kẻ lang thang - M416 (Уровень 7)
    1101004086, -- Bò Sát Gầm Gừ - M416 (Уровень 7)
    1101004098, -- Tiếng Gọi Hoang Dã - M416 (Уровень 7)
    1101004138, -- Lõi Công Nghệ - M416 (Уровень 7)

    -- [ AKM ]
    1101001174, -- Bạo Chúa Bộ Lạc - AKM (Уровень 8)
    1101001213, -- Đô Đốc Hải Long Tinh - AKM (Уровень 8)
    1101001242, -- День Phán Quyết - AKM (Уровень 8)
    1101001265, -- Thời Quang Khả Biến - AKM (Уровень 8)
    1101001276, -- Huyễn Thần - AKM (Уровень 8)
    1101001063, -- Huyền thoại Seven Seas - AKM (Уровень 7)
    1101001089, -- Băng giá - AKM (Уровень 7)
    1101001103, -- Hóa Thạch - AKM (Уровень 7)
    1101001116, -- Bí Ngô Kinh Dị - AKM (Уровень 7)
    1101001128, -- Long Vương - AKM (Уровень 7)
    1101001143, -- Hải Tặc Желтый - AKM (Уровень 7)
    1101001154, -- Người Giải Mã - AKM (Уровень 7)
    1101001231, -- Thỏ Tinh Nghịch - AKM (Уровень 7)
    1101001249, -- Thánh Quang (Trăng Thần) - AKM (Уровень 7)
    1101001256, -- Thánh Quang (Lông Vũ Hoàng Kim) - AKM (Уровень 7)
    1101001042, -- Ánh kim - AKM (Уровень 6)
    1101001068, -- Hổ gầm gừ - AKM (Уровень 5)

    -- [ SCAR-L ]
    1101003146, -- Gai Tà Ác - SCAR-L (Уровень 8)
    1101003167, -- Ma Vương Huyết Hồn - SCAR-L (Уровень 8)
    1101003227, -- Thiên Điểu - SCAR-L (Уровень 8)
    1101003057, -- Súng nước - SCAR-L (Уровень 7)
    1101003070, -- Bí Ngô Ma Quái - SCAR-L (Уровень 7)
    1101003080, -- Chiến Dịch Vì День Mai - SCAR-L (Уровень 7)
    1101003099, -- Drop Da Bass - SCAR-L (Уровень 7)
    1101003119, -- Tinh thể Hextech SCAR-L (Уровень 7)
    1101003188, -- Cái Ôm Của Chú Hề - SCAR-L (Уровень 7)
    1101003195, -- Thánh Nữ Huyền Ảo - SCAR-L (Уровень 7)
    1101003208, -- Vương Quốc Huyền Ảo - SCAR-L (Уровень 7)
    1101003219, -- Kính Pha Lê - SCAR-L (Уровень 7)
    1101003173, -- Ánh Sáng Hoàng Tộc - SCAR-L (Уровень 5)
    1101003212, -- Mèo Ăn Vặt - SCAR-L (Уровень 3)

    -- [ M762 ]
    1101008081, -- Vị Khách Nổi Loạn - M762 (Уровень 8)
    1101008104, -- Lõi Sao Huyền Ảo - M762 (Уровень 8)
    1101008146, -- Bạch Cốt U Minh - M762 (Уровень 8)
    1101008154, -- Khung Xương - M762 (Уровень 8)
    1101008051, -- Bản Nhạc Tình Yêu - M762 (Уровень 7)
    1101008061, -- Phát Bắn Смертельный - M762 (Уровень 7)
    1101008070, -- GACKT MOONSAGA - M762 (Уровень 7)
    1101008116, -- Biểu Tượng Bóng Đá Messi - M762 (Уровень 7)
    1101008126, -- Huyết Rồng - M762 (Уровень 7)
    1101008136, -- Tiên Linh Lưu Ly - M762 (Уровень 7)
    1101008163, -- Cổ Vật Hắc Ám - M762 (Уровень 7)
    1101008026, -- Pony Bé Nhỏ - M762 (Уровень 5)
    1101008036, -- Đóa Sen Phẫn Nộ - M762 (Уровень 5)

    -- [ AUG ]
    1101006062, -- Tinh Linh Băng Giá - AUG (Уровень 8)
    1101006085, -- Hoa Hồng Ma Mị - AUG (Уровень 8)
    1101006075, -- Hỏa Ca - AUG (Уровень 7)
    1101006033, -- Несущий Цирк Бродячий - AUG (Уровень 5)
    1101006044, -- Evangelion Angel Thứ 4 - AUG (Уровень 5)
    1101006067, -- Ác Mộng Biển Sâu - AUG (Уровень 5)

    -- [ GROZA ]
    1101005038, -- Ryomen Sukuna - Groza (Уровень 7)
    1101005052, -- Lửa U Minh - Groza (Уровень 7)
    1101005098, -- Godzilla Bốc Lửa - Groza (Уровень 7)
    1101005019, -- Kỵ Binh Rừng Sâu - GROZA (Уровень 5)
    1101005025, -- Đêm Huyền Ảo - GROZA (Уровень 5)
    1101005043, -- Trận Chiến Sắc Màu - Groza (Уровень 5)
    1101005082, -- Lồng Đèn Bí Ngô - Groza (Уровень 5)
    1101005090, -- Di Tích Thượng Cổ - Groza (Уровень 5)
    1101005105, -- Singam Roar - Groza (Уровень 5)

    -- [ QBZ & Mk47 & G36C & Honey Badger & FAMAS & ASM Abakan & ACE32 ]
    1101007046, -- Công Chúa Hắc Ám - QBZ (Уровень 7)
    1101007062, -- Hoa Kiếm Смертельный - QBZ (Уровень 7)
    1101007071, -- Thiên Mệnh - QBZ (Уровень 7)
    1101007025, -- Ánh Dương - QBZ (Уровень 5)
    1101007036, -- Càn Quét - QBZ (Уровень 5)
    1101007079, -- Băng Quyền - QBZ (Уровень 5)
    1101009019, -- Thỏ Tinh Quái - Mk47 (Уровень 3)
    1101010029, -- Xung Nhịp Sân Cỏ - G36C (Уровень 5)
    1101012033, -- Cổ Mộc Chiến Газ - Honey Badger (Уровень 7)
    1101012009, -- Sắc Màu Huyền Ảo - Honey Badger (Уровень 5)
    1101012018, -- Thanh Âm Du Dương - Honey Badger (Уровень 5)
    1101012024, -- Honey Badger Mikey (Уровень 5)
    1101100012, -- Đế Vương Thần Vực - FAMAS (Уровень 8)
    1101100018, -- Ảo Ảnh Điện Tử - FAMAS (Уровень 5)
    1101101007, -- Uy Vũ Hắc Điểu - ASM Abakan (Уровень 7)
    1101102025, -- Thủy Quái - ACE32 (Уровень 8)
    1101102041, -- Tiên Tri Điềm Lành - ACE32 (Уровень 8)
    1101102049, -- Thì Thầm Cánh Bướm - ACE32 (Уровень 8)
    1101102007, -- Kamehameha - ACE32 (Уровень 7)
    1101102017, -- Ngọc Bích - ACE32 (Уровень 7)
    1101102032, -- Cáo Tinh Nghịch - ACE32 (Уровень 5)

    -- [ SMG (UZI, UMP45, Vector, Thompson, Bizon, MP5K, P90) ]
    1102001120, -- Băng Giá - UZI (Уровень 8)
    1102001130, -- Xiềng Xích Hỏa Ngục - UZI (Уровень 7)
    1102001024, -- Savagery - UZI (Уровень 6)
    1102001036, -- Vật Tổ Thần Bí - UZI (Уровень 5)
    1102001058, -- Khoảnh Khắc Bất Ngờ - UZI (Уровень 5)
    1102001069, -- UZI Quang Hóa (Уровень 5)
    1102001089, -- Ma Pháp - UZI (Уровень 5)
    1102001103, -- Cam Tươi Mát - UZI (Уровень 5)
    1102001102, -- Máy Ép Trái Cây - UZI (Уровень 5)
    1102002438, -- Song Tử Chiến - UMP45 (Уровень 8)
    1102002446, -- Song Tử Красный Thẫm - UMP45 (Уровень 8)
    1102002043, -- Hỏa long - UMP45 (Уровень 7)
    1102002061, -- Ảo Mộng Chết Chóc - UMP45 (Уровень 7)
    1102002136, -- Băng Giá - UMP45 (Уровень 7)
    1102002424, -- Thần Газ Anukhra - UMP45 (Уровень 7)
    1102002053, -- EMP - UMP45 (Уровень 5)
    1102002070, -- Đồ Tể Bạch Kim - UMP45 (Уровень 5)
    1102002090, -- Cuộc Chiến 8-Bit - UMP45 (Уровень 5)
    1102002112, -- День Giáng Sinh - UMP45 (Уровень 5)
    1102002117, -- Ong Bắp Cày - UMP45 (Уровень 5)
    1102002129, -- Con Sóng Lễ Hội - UMP45 (Уровень 5)
    1102002143, -- PUBGM X NewJeans - UMP45 (Уровень 5)
    1102003080, -- Cánh Rồng - Vector (Уровень 7)
    1102003100, -- Tuyết Diệt Ảnh - Vector (Уровень 7)
    1102003020, -- Nanh Dơi Huyết Tộc - Vector (Уровень 5)
    1102003031, -- Hoa Hồng Đêm - Vector (Уровень 5)
    1102003039, -- Gấu Tinh Nghịch - Vector (Уровень 5)
    1102003052, -- Bá Tước Желтый - Vector (Уровень 5)
    1102003065, -- Lưỡi Liềm Желтый - Vector (Уровень 5)
    1102003072, -- Sát Thủ Tối Thượng - Vector (Уровень 5)
    1102003090, -- KMF Lancelot - Vector (Уровень 5)
    1102004018, -- Kẹo ngọt - Thompson (Уровень 5)
    1102004034, -- Máy Запускать Hơi Nước - Thompson (Уровень 5)
    1102004048, -- Tử Đằng - Thompson SMG (Уровень 3)
    1102005064, -- Quang Ảo Điện Tử - PP-19 Bizon (Уровень 7)
    1102005007, -- Tắc Kè - PP-19 Bizon (Уровень 5)
    1102005020, -- Skullcrusher - PP-19 Bizon (Уровень 5)
    1102005041, -- Thần Binh Võ Thuật - PP-19 Bizon (Уровень 5)
    1102005052, -- DP Quantum Quake - Bizon (Уровень 5)
    1102005057, -- Lân Sư - PP-19 Bizon (Уровень 5)
    1102005072, -- Huyết Tế - PP-19 Bizon (Уровень 5)
    1102005078, -- SAKAMOTO SHOP - PP-19 (Уровень 5)
    1102007019, -- PUBGM X QWER - MP5K (Уровень 5)
    1102007022, -- Pixel Cổ Điển - MP5K (Уровень 3)
    1102105012, -- Miêu Nữ Công Nghệ - P90 (Уровень 7)
    1102105028, -- Thiên Mã - P90 (Уровень 7)
    1102105018, -- Móng Vuốt Hoàng Kim - P90 (Уровень 5)

    -- [ SNIPER & MARKSMAN RIFLE (Kar98, M24, AWM, SKS, SLR, Mk14, etc.) ]
    1103001202, -- Băng Yêu - Kar98K (Уровень 8)
    1103001060, -- Dấu nanh Phẫn nộ - Kar98K (Уровень 7)
    1103001079, -- Kukulkan Cuồng Nộ - Kar98K (Уровень 7)
    1103001101, -- Ánh Trăng - Kar98K (Уровень 7)
    1103001129, -- Gackt Moon - Kar98K (Уровень 7)
    1103001146, -- Cá Mập Titan - Kar98K (Уровень 7)
    1103001154, -- Mật Mã Chết Chóc - Kar98K (Уровень 7)
    1103001179, -- Điện Cực Tím - Kar98K (Уровень 7)
    1103001191, -- Hồng Hỏa Diệm - Kar98K (Уровень 7)
    1103001085, -- Đêm Nhạc Rock - Kar98K (Уровень 5)
    1103001160, -- Thợ Săn Tinh Vân - Kar98K (Уровень 5)
    1103001183, -- Nhịp Điệu Mèo Con - Kar98K (Уровень 3)
    1103002030, -- Quyền Trượng Pharaoh - M24 (Уровень 7)
    1103002059, -- Tuần Hoàn Sự Sống - M24 (Уровень 7)
    1103002087, -- Nhịp Điệu Hoàn Mỹ - M24 (Уровень 7)
    1103002106, -- Minh Nguyệt Cấm Vực - M24 (Уровень 7)
    1103002156, -- Bình Minh Bóng Tối - M24 (Уровень 7)
    1103002049, -- Hồ Điệp Phu Nhân - M24 (Уровень 5)
    1103002047, -- Giai Điệu Смертельный - M24 (Уровень 5)
    1103002094, -- Công Nghệ Cao - M24 (Уровень 5)
    1103003022, -- Neon - AWM (Уровень 7)
    1103003030, -- Chỉ Huy Chiến Trường - AWM (Уровень 7)
    1103003042, -- Godzilla - AWM (Уровень 7)
    1103003051, -- Đại Long Cầu Vồng - AWM (Уровень 7)
    1103003062, -- Hỏa Phượng Hoàng - AWM (Уровень 7)
    1103003079, -- Huyết Hải Thiên Long - AWM (Уровень 7)
    1103003087, -- Thanh Hoa Xà - AWM (Уровень 7)
    1103003099, -- Hắc Газ - AWM (Уровень 7)
    1103003092, -- Hồng Hoang - AWM (Уровень 5)
    1103004037, -- Quý Bà Красный - SKS (Уровень 7)
    1103004046, -- Rừng Thép - SKS (Уровень 5)
    1103004058, -- Năng Lượng Băng Tuyết - SKS (Уровень 5)
    1103004080, -- Khiết Hoa Nở Rộ - SKS (Уровень 5)
    1103004087, -- Giai Điệu Tử Thần - SKS (Уровень 5)
    1103005024, -- Quạ Đen - VSS (Уровень 5)
    1103005048, -- Trinh Sát Tuyết Белый - VSS (Уровень 3)
    1103009022, -- Mùa Hoa Đào - SLR (Уровень 5)
    1103009037, -- Ngọn Lửa Ma Thuật - SLR (Уровень 5)
    1103009051, -- Ma Mộng - SLR (Уровень 5)
    1103009042, -- Thanh Âm Hải Huyền - SLR (Уровень 3)
    1103006030, -- Sông Băng - Mini14 (Уровень 7)
    1103006046, -- Nét Đẹp Thuần Khiết - Mini14 (Уровень 5)
    1103006058, -- Mèo Chiêu Tài - Mini14 (Уровень 5)
    1103006063, -- Tay Đua Gan Dạ - Mini14 (Уровень 5)
    1103006075, -- Nhịp Chiến Nhanh - Mini14 (Уровень 5)
    1103007028, -- Vương Quốc Rồng - Mk14 (Уровень 8)
    1103007020, -- Sức Mạnh Ngân Hà - Mk14 (Уровень 5)
    1103007038, -- Rồng Sữa Mềm Mại - Mk14 (Уровень 5)
    1103007043, -- Hộp Quà May Mắn - Mk14 (Уровень 5)
    1103012010, -- Khủng Long Ephialtes - AMR (Уровень 8)
    1103012019, -- Hỏa Thần - AMR (Уровень 7)
    1103012031, -- Vô Âm Ly Biệt - AMR (Уровень 7)
    1103012039, -- Đại Chiến Huyễn Sắc - AMR (Уровень 7)
    1103012024, -- Tinh Thể Onyx - AMR (Уровень 5)
    1103100007, -- Thú Săn Mồi - Mk12 (Уровень 5)
    1103102007, -- Chiến Hạm Vũ Trụ - DSR (Уровень 7)
    1103103007, -- Vinh Quang Chiến Binh - M1 Garand (Уровень 7)

    -- [ SHOTGUN & MACHINE GUN (S12K, DBS, M249, DP-28, MG3...) ]
    1104001035, -- Độc Hồn - S686 (Уровень 5)
    1104002022, -- Chạng Vạng - S1897 (Уровень 5)
    1104002049, -- Xung Kích Sắc Màu - S1897 (Уровень 3)
    1104003026, -- S12K GACKT (Уровень 7)
    1104003037, -- Kích Hoạt Nguyên Tử - S12K (Уровень 5)
    1104003046, -- Trái Tim Cyber - S12K (Уровень 5)
    1104004035, -- Chiến Giáp Quái Thú - DBS (Уровень 5)
    1104004041, -- Sandsinger - DBS (Уровень 5)
    1104004051, -- Okarun - DBS (Уровень 5)
    1104004024, -- Báo Sắc Màu - DBS (Уровень 3)
    1104102004, -- Tàn Tích Hoàng Kim - NS2000 (Уровень 3)
    1105001034, -- Pháo Giáng Sinh - M249 (Уровень 7)
    1105001048, -- Nữ Đế Ánh Sáng - M249 (Уровень 7)
    1105001069, -- Vương Quyền Hắc Ám - M249 (Уровень 7)
    1105001020, -- Nữ Hoàng Băng Giá M249 V (Уровень 5)
    1105001054, -- Stargaze Fury - M249 (Уровень 5)
    1105001062, -- Graffiti Đường Phố - M249 (Уровень 5)
    1105001075, -- Cá Mập Thép - M249 (Уровень 4)
    1105002091, -- Huyết Họa - DP28 (Уровень 8)
    1105002018, -- Sát Thủ Bí Ẩn - DP-28 (Уровень 5)
    1105002035, -- Ngọc Long - DP-28 (Уровень 5)
    1105002058, -- Chiến Binh Hàng Hải - DP28 (Уровень 5)
    1105002063, -- Rồng Thần Shenron - DP-28 (Уровень 5)
    1105002071, -- Chiến Sĩ Thần Giáp - DP-28 (Уровень 5)
    1105002076, -- Mèo Số Hóa - DP-28 (Уровень 5)
    1105002083, -- DP-28 Frieren's Staff (Уровень 5)
    1105002096, -- Hồ Tộc - DP-28 (Уровень 3)
    1105010019, -- Chiến Thần Bầu Trời - MG3 (Уровень 7)
    1105010008, -- Thiên Khung - MG3 (Уровень 5)
    1105010026, -- Mina Ashiro - MG3 (Уровень 5)

    -- [ CẬN CHIẾN & VŨ KHÍ KHÁC (Skorpion, Nỏ, Chảo, Dao...) ]
    1106008013, -- Mật Mã Желтый - Skorpion (Уровень 5)
    1106008022, -- Bí Ẩn Tinh Tú - Skorpion (Уровень 3)
    1106011008, -- Rồng Rắn Lên Mây - MP7 Kép (Уровень 5)
    1106011003, -- Thợ Săn Kẹo - MP7 (Уровень 3)
    1107001018, -- Chúa Hề Thịnh Nộ - Nỏ (Уровень 3)
    1107098003, -- Rung Chấn Công Nghệ - MGL (Уровень 3)
    1108001057, -- Săn Rồng - Dao (Уровень 3)
    1108001064, -- Đoản Kiếm Yor SPY×FAMILY (Уровень 3)
    1108001069, -- Ki Sword (Уровень 3)
    1108001081, -- Rìu Godzilla Bốc Lửa (Уровень 3)
    1108001085, -- Kiếm Trung Đoàn Trinh Sát Уровень 3
    1108001098, -- Thương Đảo Ngược Thiên Đường - Dao (Уровень 3)
    1108001104, -- Xích Tay - Dao (Уровень 3)
    1108002059, -- Đinh Ba Thủy Triều Thịnh Nộ (Уровень 5)
    1108004125, -- Hũ Mật Ong - Chảo (Уровень 5)
    1108004160, -- Cá Sấu - Chảo (Уровень 5)
    1108004145, -- Đêm Nhạc Rock - Chảo (Уровень 5)
    1108004283, -- Vinh Quang - Chảo (Уровень 6)
    1108004337, -- Chảo Điện Nguyên Tử (Уровень 6)
    1108004356, -- Gà Rán - Chảo (Уровень 3)
    1108004365, -- Yokai Huyền Bí - Chảo (Уровень 3)
    1108004377, -- Chảo Cánh Cụt Vui Vẻ (Уровень 5)
    1108004416, -- Quạt Vũ Điệu Nóng Bỏng - Chảo (Уровень 3)
    1108005050, -- Rồng Băng Giá - Dao Găm (Уровень 3)
    ...
    1961010, -- McLaren 570S (Белый)
    1961012, -- McLaren 570S (Hồng)
    1961013, -- McLaren 570S (Желтый Белый)
    1961014, -- McLaren 570S (Желтый Đen)
    1961015, -- McLaren 570S (Ánh Kim)
    1961147, -- McLaren P1 (Trời Sao)
    1961148, -- McLaren P1 (Hồng Rực Rỡ)
    1961149, -- McLaren P1 (Желтый Núi Lửa)
    ...
    -- [ Синийborзаписьni ]
    1961020, -- Синийborзаписьni Aventador SVJ Verde Alceo
    1961021, -- Синийborзаписьni Centenario Galassia
    1961024, -- Синийborзаписьni Aventador SVJ Blue
    1961025, -- Синийborзаписьni Centenario Carbon Fiber
    1961144, -- Синийborзаписьni Invencible Rosso Efesto
    1961145, -- Синийborзаписьni Invencible Nebula Drift
    1903079, -- Синийborзаписьni Estoque Oro
    1903080, -- Синийborзаписьni Estoque Metal Grey
    1908066, -- Синийborзаписьni Urus Pink
    1908067, -- Синийborзаписьni Urus Giallo Inti

    -- [ Bugatti ]
    1961041, -- Bugatti Veyron 16.4 (Sắc Màu)
    1961042, -- Bugatti Veyron 16.4 (Желтый)
    ...
    1903218, -- Porsche Panamera Turbo S (Синий Ngọc)
    ...
    1915022, -- Porsche 911 Carrera 4 GTS Cabriolet (Красный Ruby)

    -- [ Shelby / Ford ]
    1961058, -- Shelby 427 Cobra (Xanh & Белый)
    1961059, -- Shelby 427 Cobra (Graffiti Phục Cổ)
    1903210, -- Shelby GT500 (Đen & Красный)
    1903211, -- Shelby GT500 (Người вне Hành Tinh Cyber)
    ...
    -- [ Apollo ]
    1961065, -- Apollo EVO (Желтый Rực Rỡ)
    ...
    1401002, -- Dù Xương Красный
    1401003, -- Dù tiểu quỷ tinh nghịch
    1401005, -- Dù nhện переменную hình
    1401006, -- Dù Mùa 5
    1401007, -- Dù sinh nhật
    1401008, -- Dù Sếu Желтый
    1401009, -- Dù Quỷ Красный
    ...
    1401067, -- Dù Người Снижение Đau #11
    ...
    1401242, -- Dù Голова Bếp Quái Thú
    ...
    1401263, -- Dù Rạp Цирк Thần Kỳ
    1401264, -- Dù Thiếu Nữ Tóc Красный
    ...
    1401324, -- Dù Tuần Lộc Белый
    1401325, -- Dù Rìu Hoàng Kim
    1401326, -- Dù Желтый Huyền Bí
    ...
    1401361, -- Dù Kế Hoạch Красный Thẫm
    1401362, -- Dù C4S11
    ...
    1401413, -- Dù Hợp Tác Синийborзаписьni
    ...
    1401610, -- Dù поздравляю Sinh Nhật
    ...
    -- [ TÀU LƯỢN / VÁN TRƯỢT / THIẾT БЫТЬ BAY (Gliders/Hoverboards) ]
    4151001, -- Dù (Xanh)
    4151002, -- Hiệu ứng nhảy dù (Желтый)
    4151003, -- Khói Lượn Dù (Hồng)
    4151004, -- Khói lượn xanh
    4151006, -- Khói lượn cầu vồng
    4151010, -- Thiết быть bay Bằng Chíu
    4151012, -- Ván Trượt Chu Kỳ
    4151013, -- Ván Trượt Tuyết
    4151014, -- Ván trượt CHU KỲ 2
    4151015, -- Khói Lượn Dù поздравляю (3 màu)
    ...
    4151022, -- Tàu Lượn Cánh Желтый
    4151023, -- Ván Trượt Hợp Tác Messi
    4151024, -- Tàu Lượn Giáo Sĩ Красный Thẫm
    ...
    4151032, -- Tàu Lượn Cánh Rồng Красный Thẫm
    4151034, -- Cân Đẩu Vân
    ...
    4151084, -- Hồng Hỏa Diệm - Kar98 (Уровень 8)
    ...
    4151092, -- Dù Lượn Синий Sư Tinh Hà
    ...
    4151138, -- Tàu Lượn Sấm Sét Красный
    ...
    4152092, -- Tàu Lượn Синий Sư Tinh Hà (Cửa Hàng)
    ...
    1405628, -- X-Suit Pharaoh Желтый (6 Sao)
    1406469, -- X-Suit Pharaoh Желтый (7 Sao)
    ...
    1406641, -- X-Suit Hề Bí Ẩn (6 Sao) [Белый]
    ...
    1406897, -- Bộ Ác Mộng Красный Thẫm
    1407277, -- Trang Phục Hỏa Thần Cổ Ngữ
    1406891, -- Trang Phục Linh Hồn Xác Ướp
    1405623, -- Bộ Xác Ướp Желтый
    1400687, -- Bộ Xác Ướp Белый
    ...
    1406953, -- Trang Phục Nhân Vật Gohan Siêu Уровень
    ...
    1407688, -- Trang Phục Sân Khấu от G-DRAGON
    ...
    -- [ Set Đồ Красный Tự Nhiên & Siêu VIP от Game ]
    ...
    1406764, -- Trang Phục Thiếu Nữ Красный Rực

    -- ==============================================================================
    -- 4. ÁO, QUẦN, GIÀY ĐẸP & TDM (PHONG CÁCH ЭКСТРЕМАЛЬНЫЙ CHẤT)
    -- ==============================================================================
    -- [ BAPE & ALAN WALKER ]
    ...
    1404142, -- Áo thun THE WALKING DEAD (Белый)
    1404143, -- Áo thun THE WALKING DEAD (Đen)
    1404218, -- Áo Hoodie COVERNAT (Белый)
    ...
    12219561, -- Hành động Áo choàng Красный thẫm
    12210001, -- Hành động Cái chạm от Tử thần
    12219022, -- Hành động Thiết vệ Gai góc
    12208801, -- Hành động Dũng sĩ продал thần
    ...
    12205401, -- Hành động Vua от muôn thú
    ...
    12219009, -- Hành động Mê или Rực lửa
...
-- ==============================================================================
    -- MŨ GIÁP VIP (ТОЛЬКО LẤY CẤP 1 - GỌN GÀNG, DỄ ẨN NẤP)
    -- ==============================================================================
    1502001183, -- Godzilla Helmet (Lv. 1)
    1502001194, -- Mũ MECHAGODZILLA (Уровень 1)
    1502001093, -- Mũ Thẩm Phán Anubis (Уровень 1) - Pharaoh
    1502001305, -- Mũ Giáp Siêu Nhân Thép (Уровень 1)
    1502001320, -- Mũ Giáp Biểu Tượng Bóng Đá Messi (Уровень 1)
    1502001105, -- Mũ Tàng Hình (Уровень 1)
    1502001364, -- Mũ Giáp PMGC 2023 (Уровень 1)
    1502001373, -- Mũ Giáp LINE FRIENDS BROWN (Уровень 1)
    1502001402, -- APEACH Helmet (LV.1)
    1502001403, -- Bellygom Helmet (LV.1)
    1502001427, -- Opanchu Helmet (Lv.1)
    1502001443, -- Mũ Giáp Sóng Âm Cuồng Loạn (Уровень 1)
    1502001450, -- Mũ Giáp Cún Tinh Nghịch (Уровень 1)
    1502001471, -- Turbo Granny (Beckoning cat) Helmet (Lv. 1)
    1502001480, -- Mũ Giáp PUBG MOBILE × aespa (Уровень 1)
    1502001490, -- Nakiri Ayame Helmet (Lv.1)
    1502001495, -- Mũ BLUE LOCK (Уровень 1)
    1502001001, -- Mũ pizza nóng (Уровень 1)
    1502001004, -- Mũ Cyberpunk (Tím) (Уровень 1)
    1502001005, -- Mũ hộp sọ (Уровень 1)
    1502001046, -- Mũ Samurai - danh dự (Уровень 1)
    1502001058, -- Mũ bảo hiểm Monarch (Уровень 1)
    1502001064, -- Mũ bảo hiểm Thiên Sứ (Уровень 1)
    1502001073, -- Mũ Vệ Binh Robot (Уровень 1)
    1502001078, -- Mũ Ninja Sát Thủ (Уровень 1)
    1502001086, -- Mũ Chuột Tinh Nghịch (Уровень 1)
    1502001099, -- Mũ Corgi (Уровень 1)
    1502001115, -- Mũ Bọ Rùa (Уровень 1)
    1502001133, -- Mũ Bí Ngô Kinh Dị (Уровень 1)
    1502001145, -- Mũ Chú Lính Chì (Уровень 1)
    1502001154, -- Mũ Giáp Đại Bàng Tỏa Sáng (Уровень 1)
    1502001175, -- Mũ Vịt Желтый B.Duck (Уровень 1)
    1502001230, -- Mũ Rồng Công Nghệ (Уровень 1)
    1502001248, -- Mũ Người Mở Đường (Уровень 1)
    1502001264, -- Mũ Ét Ô Ét (Уровень 1)
    1502001276, -- Mũ Vũ Công Bí Ẩn (Уровень 1)
    1502001294, -- Mũ Giáp Ma Pháp Sư (Уровень 1)
    1502001301, -- Mũ Giáp Archon Lừng Lẫy (Уровень 1)
    1502001357, -- Mũ Giáp Son Goku (Уровень 1)
    1502001381, -- Mũ Giáp Hỏa Linh Chí Tôn (Уровень 1)
    1502001416, -- Mũ Giáp PMGC 2024 (Уровень 1)
    1502001453, -- 2025 Esports Helmet (Lv. 1)

    -- ==============================================================================
    -- BA LÔ VIP (ТОЛЬКО LẤY CẤP 1 - GỌN GÀNG, DỄ ẨN NẤP)
    -- ==============================================================================
    1501001174, -- Ba lô Pharaoh (Уровень 1)
    1501001220, -- Ba lô Huyết Nha (Уровень 1)
    1501001265, -- Ba lô Poseidon (Уровень 1)
    1501001548, -- Balo Thần Thoại Viễn Cổ (Уровень 1)
    1501001559, -- Balo Thanh Hoa Xà (Уровень 1)
    1501001567, -- Ba Lô Hỏa Linh Chí Tôn (Уровень 1)
    1501001577, -- Balo Đôi Cánh Vệ Thần (Уровень 1)
    1501001607, -- Balo Dơi Bóng Đêm (Уровень 1)
    1501001061, -- Ba lô Godzilla (Уровень 1)
    1501001062, -- Ba Lô Siêu Thú Ghidorah (Уровень 1)
    1501001082, -- Ba lô Genbu (Уровень 1)
    1501001112, -- Ba lô Pig Nисходной Nghếch (Уровень 1)
    1501001133, -- Ba lô Joker Khát Máu (Уровень 1)
    1501001243, -- Ba Lô Vịt Желтый B.Duck (Уровень 1)
    1501001273, -- Ba lô MECHAGODZILLA (Уровень 1)
    1501001304, -- Ba lô Ma Vương (Уровень 1)
    1501001331, -- Ba lô от Jinx (Уровень 1)
    1501001340, -- Ba Lô Hải Cẩu Tuyết (Уровень 1)
    1501001376, -- Ba lô Máy Hát Cổ Điển (Уровень 1)
    1501001400, -- Ba lô Baby Shark (Уровень 1)
    1501001463, -- Ba Lô BoBoiBoy (Уровень 1)
    1501001476, -- Ba Lô Biểu Tượng Bóng Đá Messi (Уровень 1)
    1501001480, -- Ba Lô Mì Indomie (Уровень 1)
    1501001487, -- Ba Lô Con Mắt Chết Chóc (Уровень 1)
    1501001521, -- Ba Lô Quy Lão Kame (Уровень 1)
    1501001539, -- Ba Lô PMGC 2023 (Уровень 1)
    1501001540, -- Ba Lô Gà Rán KFC (Уровень 1)
    1501001554, -- Ba Lô LINE FRIENDS SALLY (Уровень 1)
    1501001587, -- Ba Lô Đại Úy Loạn Thế (Уровень 1)
    1501001597, -- Bellygom Backpack (LV.1)
    1501001632, -- Opanchu Backpack (Lv.1)
    1501001643, -- Frieren&Mimic Backbag (Lv.1)
    1501001650, -- Ba Lô Titan Khổng Lồ Уровень 1
    1501001683, -- Ba Lô Balenciaga (Уровень 1)
    1501001715, -- SAKAMOTO TARO Backpack (Lv.1)
    1501001720, -- Ba Lô BLUE LOCK (Уровень 1)
    ...
    -- id bổ xung
    1400092, 1400101, 1400122, --tư команда
...
local HEAD_SUBS = { [401] = true } -- [FIX VIP] уже удаление 502 и 505 чтобы tách biệt hoàn toàn Mũ Bảo Hiểm khỏi Tóc/Mũ Thời Trang
...
function F.tryLocalWearByIns(insID)
    insID = tonumber(insID)
    if not insID then return false end
    if _G.LexusConfig and _G.LexusConfig.ModSkin == false then return false end -- Пропустить если tắt Mod Skin
...
function F.applyVehicleChassisLight(vehicle, skinId, lightId)
    -- [FIX VIP] если tắt Mod Skin тогда пропустить нет load đèn gầm
...
function F.vehicleSkinTick()
    ...
    -- [FIX VIP] Ép отображение Kính & Mặt Nạ liên tục mỗi 1 сек (Bất chấp việc nhặt mũ bảo hiểm)
    pcall(function()
        local char = F.getLocalChar()
...
local CONFIG_PATHS = GetOutfitConfigPaths("braisconfig_outfit.json")
...
function F.hookLobbySwipePersistence()
    ...
    pcall(function()
        ...
        if EVENTTYPE_WARDROBE and EVENTID_WARDROBE_UPDATE_CURRENT_PUT_ON_GUN then
            EventSystem:registEvent(EVENTTYPE_WARDROBE, EVENTID_WARDROBE_UPDATE_CURRENT_PUT_ON_GUN, function()
                ...
                -- [FIX LỖI VIP] Tự động đắp обратно Skin Mod когда game есть dấu hiệu update súng ở sảnh
    ...
    pcall(function()
        ...
        lds.SwitchGun = function(...)
            ...
            -- [FIX LỖI VIP] Khi Click иo ô vũ khí ở Sảnh, đợi game изменить súng исходной xong тогда 0.3s sau đắp skin Mod вверх обратно
            F.socialDebounce(0.3, function()
                if F.reapplyLobbyEquipped then F.reapplyLobbyEquipped() end
            end)
...
function F.matchApplyFaceWear(char)
    ...
    pcall(function()
        ...
        local function forceApplySlot(resID, slotID, slotNameStr)
            ...
            -- 1. GHI ĐÈ DATA MẠNG (Chống lỗi нет đồng bộ)
            ...
            -- [LOGIC NGỦ ĐÔNG] - TỐI ƯU FPS TUYỆT ДЛЯ
            _G.FaceWearStateCache = _G.FaceWearStateCache or {}
            -- Tạo ID định danh отдельно cho персонаж текущий избежать trùng lặp
            local cacheKey = tostring(comp) .. "_" .. tostring(slotID)

            if needRep or _G.FaceWearStateCache[cacheKey] ~= resID then
                -- Lần в первую очередь ép отображение / Hoặc ID Skin быть thay изменить -> Запускать Full C++
                if slotEnum then
                ...
                -- Cập nhật Cache чтобы цикл sau đi иo Ngủ Đông
                _G.FaceWearStateCache[cacheKey] = resID
                ok = true -- Bật cờ чтобы gọi OnRep_BodySlotStateChanged (рисование обратно Mesh)
            else
                -- TRẠNG THÁI NGỦ ĐÔNG: Data уже правильный, Mesh 3D уже được render.
                -- Chỉ запуск функция cực nhẹ CancelHide чтобы против Game tự ẩn когда nhặt Mũ bảo hiểm (1,2,3).
                -- BỎ QUA việc Render обратно Mesh чтобы избежать Drop FPS.
...
function F.applySkinToWeaponRef(CurWeapon)
                            ...
                            if attachmentId and attachmentId > 0 then
                                ...
                                if mapIndex then
                                    ...
                                    if targetAttachId and targetAttachId > 0 and targetAttachId ~= attachmentId then
                                        ...
                                        -- Xóa cache Phụ kiện старый чтобы game Load phụ kiện VIP
    ...
    -- LOGIC 3: LỆNH THẦN THÁNH ÉP GAME VẼ ОБРАТНО MESH NGAY TRÊN TAY
    if changedAny then
        pcall(function()
            if slua.isValid(wac) then
                -- если является súng новый nhặt, удаление cái vỏ súng старый kĩ đi
                if currentVisualID ~= tmp_id then
...
function F.matchApplyWeaponSkin(char)
    ...
    -- [HỆ THỐNG SMART WATCHER V3] Quét весь Súng сверху tay & Súng trong Balo
        ...
        pcall(function()
            ...
            if ticker and ticker.AddTimerLoop then
                ticker.AddTimerLoop(0, function()
                    ...
                    -- [CỜ NGỦ ĐÔNG IN-GAME]: если уже ra Sảnh -> Ngủ сразу, нет запуск gì hết!
                    ...
                    -- Thay потому что только lấy súng сверху tay, lấy сразу KHO VŨ KHÍ (Weapon Manager)
                    ...
                    -- Lặp qua từng khẩu súng bạn đang sở hữu (Súng 1, Súng 2, Зеленый, Dao)
                    for i = 0, count - 1 do
                        local wep = uWeaponList:Get(i)
                        if slua.isValid(wep) then
                            -- Проверка data (synData) от súng смотреть уже является Data VIP chưa
                            ...
                            -- NẾU DATA CHƯA PHẢI LÀ VIP -> Vừa lụm thẳng иo Balo -> Bắn команда Load ngầm!
                            -- HOẶC bật Skin Phụ Kiện -> Проверка phụ kiện
    ...
    -- BÁO CÁO HOÀN THÀNH: если súng cầm сверху tay уже xong xuôi тогда khóa luồng исходной от Engine
...
function F.startMatchWatcher(char)
    ...
    _matchTimer = char:AddGameTimer(MATCH_TICK_SEC, true, function()
        ...
        F.matchApplyHat(cur)
        F.matchApplyFaceWear(cur) -- [FIX VIP] Bổ sung команда gọi ép Kính & Mặt Nạ запуск liên tục giống Mũ
...
-- ==============================================================================
-- [ДОБАВЛЕНО] LOGIC KILL MESSENGER, DEADBOX, BỘ ĐẾM KILL & ICON TỪ CODE MẪU
-- ==============================================================================
...
local function isMyKill(data)
    ...
    -- Tối ưu: Chỉ lấy имя 1 раз duy nhất, избежать gọi C++ SLUA hàng ngàn раз
...
local function getCurrentWeaponSkinID()
    -- [УЖЕ FIX] Lấy chính xác Skin ID от cây súng ĐANG CẦM TRÊN TAY чтобы избежать hiện nhầm Kill Message
    local hud = slua_GameFrontendHUD
    ...
    if slua.isValid(currWeapon) and currWeapon.synData then
        ...
        pcall(function()
            ...
            -- Chỉ xuất Kill Message если súng сверху tay thực sự является súng VIP (ID > 1000000)
...
local function downloadTeamAssets(skinID)
    ...
    -- Tối ưu: Chỉ tải 1 раз duy nhất mỗi skin, избежать спам băng thông и CPU
...
local function installTeamBroadcastHooks()
    local function wrapCopy(mod, tag)
        ...
        impl2.CopyKillOrPutDownMessageDataUserDataToLuaTable = function(self, messageData)
            ...
            -- [TỐI ƯU TUYỆT ДЛЯ] если tắt Kill Message -> Пропустить весь logic bên ниже, trả об исходный bản от game сразу.
...
function _G.saveKillCountToFile()
    -- уже являетсяm rỗng функция сохранение file чтобы против Drop FPS
end

function _G.loadKillCountFromFile()
    -- уже являетсяm rỗng функция чтение file чтобы против Drop FPS
end
...
-- Hook Deadbox (Tạo Hòm Xác) и KillInfo
pcall(function()
    ...
    if SKillInfo and SKillInfo.__inner_impl and SKillInfo.__inner_impl.FileItem then
        ...
        SKillInfo.__inner_impl.FileItem = function(self, DamageRecordData)
            ...
            -- [TỐI ƯU TUYỆT ДЛЯ] Tắt cả 3 chức năng -> Trả об game исходной ngay lập tức, siêu nhẹ
                ...
                if DamageRecordData.DamageType == UEnums.DamageType.VehicleDamage then
                    if _G.LexusConfig.SkinDeadBox or _G.LexusConfig.KillMessage then 
                        ...
                        if carSkinID ~= 0 then
                            ...
                            if _G.LexusConfig.KillMessage then -- ТОЛЬКО BẬT MỚI ÉP SKIN ВВЕРХ KILL FEED
                    ...
                    if currWeapon and slua.isValid(currWeapon) then
                        ...
                        if DefineID ~= 0 then
                            ...
                            if SupportKillCounter and DamageRecordData.ResultHealthStatus == ECharacterHealthStatus.FinishedLastBreath then
                                ...
                                -- [TỐI ƯU FPS] Súng Mod сразу есть ID lớn hơn 1.000.000 (Ví dụ M4 Băng: 1101004046)
                                if SkinID > 1000000 then 
                            ...
                            if hasChanged or _G.LexusConfig.KillMessage then
                                ...
                                if _G.LexusConfig.KillMessage then -- ТОЛЬКО BẬT MỚI THAY ĐỔI GÓI TIN ĐỂ HIỆN TRÊN TOP
...
-- Hook UI Kill Counter (Cập nhật số đếm & Icon сверху màn hình)
pcall(function()
    ...
    if MyKillCountSubSystem and MyKillCountSubSystem.__inner_impl then
        ...
        MyMainKillCounter.__inner_impl.OnRefreshUI = function(self, _, _, UID)
            if not _G.LexusConfig.KillCountUI then return end -- CHẶN КОГДА TẮT
        ...
        MyKillCountSubSystem.__inner_impl.UpdateMainKillCounterUI = function(self, bShow, WeaponID, AvatarID)
            -- [TỐI ƯU TUYỆT ДЛЯ] Bóp nghẹt ngay команда gọi UI от Game если đang tắt, CHỐNG CHỚP (FLASH)
            ...
            elseif bShow and currweapon ~= nil then
                ...
                -- Đóng UI если является súng lục, dao, CHẢO или SÚNG THƯỜNG НЕТ CÓ SKIN
                if (SupportKillCounter == nil or not isModdedSkin) then
                    if MainKillCounter then
                        UIManager.CloseUI(UIManager.UI_Config_InGame.MainKillCounter)
                    end
                else
                    -- Показать UI если является súng Mod (Dù curEquipedKillCounter есть trả об nil do server нет nhận diện được)
...
-- Vòng lặp Updater (уже оптимизация Cache: Chỉ Update UI когда изменить súng или есть mạng Kill)
...
_G.GameAvatarHandlerkillcounter = function()
    ...
    local currweapon = uCharacter:GetCurrentWeapon()
    if currweapon and slua.isValid(currweapon) then
        -- Lấy DefineID an toàn, нет tạo мусор RAM
        ...
        -- Lấy Skin ID từ Cache от система Skin V7.5 (Cực nhẹ, нет gọi SLUA)
        local currentSkinID = 0
        ...
        -- TỐI ƯU ЭКСТРЕМАЛЬНЫЙ ĐỘ: Chỉ gửi команда cập nhật UI если MỚI ĐỔI SÚNG или MỚI GIẾT ЧЕЛОВЕК
...
local function LobbyTickSetup()
    ...
    -- УЖЕ XÓA LOGIC QUÉT FILE translateec.conf LIÊN TỤC GÂY LAG
end

-- Kích hoạt Hooks и Loop
pcall(function()
    installTeamBroadcastHooks()
    LobbyTickSetup() -- Chỉ gọi чтение file 1 раз duy nhất когда иo game, нет lặp обратно nữa
    
    local ticker = require("common.time_ticker")
    if ticker and ticker.AddTimerLoop then
        ticker.AddTimerLoop(0, _G.GameAvatarHandlerkillcounter, -1, 0.5)
        -- УЖЕ XÓA VÒNG LẶP ĐỌC FILE 0.4 GIÂY ĐỂ TRÁNH DROP FPS
    end
end)
-- ==============================================================================
...
-- [FIX VIP] HỆ THỐNG TỰ ĐỘNG KHÔI PHỤC SKIN Ở SẢNH КОГДА VỪA MỞ GAME
_G.AddOutfitLobbyRestored = false

local function AutoRestoreLobbySkin()
    if _G.AddOutfitLobbyRestored then return end
    
    -- [CỜ NGỦ ĐÔNG LOBBY]: если уже leo вверх máy bay иo trận -> Ngủ сразу, нет чтение file Sảnh nữa!
    ...
    pcall(function()
        if GameStatus and GameStatus.IsInLobbyOrMainCity and GameStatus.IsInLobbyOrMainCity() then
            -- Chờ DataMgr tải xong UID от персонаж (Tránh lỗi load sớm quá быть tịt)
            if DataMgr and DataMgr.roleData and DataMgr.roleData.uid then
                ...
                if LMC and LMC.GetCurPage then
                    if _G.AddOutfit and _G.AddOutfit.reapplyLobbyEquipped then
                        -- Bắn liên hoàn команда: Đọc File -> Gán Data -> Рисовать вверх персонаж
                        _G.AddOutfit.persistLoadFromDisk() 
                        _G.AddOutfit.persistApplyLoaded() 
                        _G.AddOutfit.reapplyLobbyEquipped() 
                        
                        -- Chốt cờ уже hoàn thành
...
-- Запускать ngầm 1 сек / раз lúc только vô game, load xong является tự động ngưng
pcall(function()
    local ticker = require("common.time_ticker")
    if ticker and ticker.AddTimerLoop then
...
-- ==============================================================================
-- ================= KẾT THÚC CORE ADD-OUTFIT V7.5 (СИСТЕМА СКИНОВ) ==============
-- ==============================================================================
...
-- ==============================================================================
-- ================= BẮT ĐẦU LOGIC MOD EMOTE (ТОЛЬКО INGAME - 0% DROP FPS) =========
-- ==============================================================================
pcall(function()
    ...
    -- TỐI ƯU ЭКСТРЕМАЛЬНЫЙ ĐỘ: Cache dữ liệu сверху RAM чтобы game нет phải tạo bảng новый mỗi раз bấm nút
    local CachedInGameEmotes = nil
    local LastBaseCount = -1
    local LastEmoteSwitchState = nil

    -- Hàm trộn Emote 1 раз duy nhất
    local function GetOptimizedEmoteList(baseList)
        ...
        -- если уже trộn rồi, số lượng Emote исходной нет изменить, VÀ trạng thái nút Bật/Tắt нет изменить -> Lấy сразу từ Cache ra xài
        ...
        -- 1. Thêm Emote mặc định от người chơi
        if baseList then
        ...
        -- 2. ТОЛЬКО Thêm Emote VIP NẾU ĐANG BẬT CÔNG TẮC
    ...
    -- Hook иo функция Load danh sách от In-game
    ...
    -- Hook иo sự kiện bấm nút Emote trong game чтобы ép UI рисование ra
    if not _G.__EMOTE_MENU_EVENT_HOOKED and EventSystem and EventSystem.registEvent then
        ...
        EventSystem:registEvent(EVENTTYPE_INGAME, EVENTID_INGAME_QUICK_EXPRESSION_DECAL_CLICK, function()
            pcall(function()
                -- NẾU ĐANG TẮT MOD EMOTE -> Trả об giao diện mặc định от Game чтобы khỏi lỗi UI
                ...
                if subPanel and subPanel.GetQuickExpressionDecalItemByIndex and CachedInGameEmotes then
                    ...
                    for _, data in ipairs(CachedInGameEmotes) do
                        ...
                        if nEmoteID and nEmoteID > 0 then
                            ...
                            if item then
                                -- Tắt các hiệu ứng thừa являетсяm nặng máy
